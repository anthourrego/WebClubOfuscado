...
function peticionCajaCongelada(congelar, adicional) {
	$.ajax({
		...
		data: {
			...
			adicional,
			RASTREO: RASTREO(
				congelar == "S" ? "Descongela Caja" : "Congela Caja",
				"Panel Principal"
			),
		},
...
function spawnNotification(theBody, theTitle) {
	var options = {
		body: theBody,
		icon:
			base_url() + "uploads/" + NIT() + "/InformacionHotel/logo_cliente.png",
	};
...
function notifyMe(alerta) {
	...
	if (!("Notification" in window)) {
		alertify.error(
			"Este navegador no soporta las notificaciones de escritorio."
		);
	}
...
alertify.myConfig ||
	alertify.dialog("myConfig", function () {
		var autoConfirm = {
			timer: null,
			index: null,
			text: null,
...
alertify.myConfigAlert ||
	alertify.dialog("myConfigAlert", function () {
		return {
...
function primerFocus(validarEventoFocus, currentModal) {
	/*switch (validarEventoFocus) {
		case 2:
			setTimeout(function () {
				selectedInput = currentModal
					.find(
						"input[type=search],input[type=text], input[type=date],textarea,select, div .chosen-container"
					)
					.filter(":visible");
				if (selectedInput.length <= 0) return;
				for (var select of selectedInput) {
					if (!select.disabled) {
						selectedInput = select.id;
						break;
					}
				}
				selectedInput = "#" + selectedInput;
				selectedClass = $(".modal").find("input:eq(0)").attr("class")
					? $(".modal").find("input:eq(0)").attr("class")
					: "";
				if (selectedInput.includes("chosen")) {
					firstin = $(selectedInput + " input:first");
					firstin.focus();
				} else {
					$(selectedInput).focus();
				}
				if (
					selectedInput == "#tipEvento" ||
					selectedClass.includes("chosen") ||
					selectedClass.includes("datepicker") ||
					selectedClass.includes("datetimepicker") ||
					$(selectedInput).is("select") ||
					showTeclado == false
				) {
		...
		default:
			setTimeout(function () {
				...
				selectedClass = $(document).find("input:eq(0)").attr("class")
					? $(document).find("input:eq(0)").attr("class")
					: "";
				selectedInput = $(document)
					.find("input[type=search],input[type=text],textarea,select")
					.filter(":visible:first")
					.attr("id");
				if (!selectedInput) {
					selectedInput = document.querySelector(".form-control");
					if (selectedInput != null) selectedInput = selectedInput.id;
				}
				selectedInput = "#" + selectedInput;
				$(selectedInput).focus();
				if (
					selectedInput == "#valProducto" ||
					selectedClass.includes("chosen") ||
					selectedClass.includes("datepicker") ||
					selectedClass.includes("datetimepicker") ||
					$(selectedInput).is("select") ||
...
(function () {
	document.addEventListener("DOMContentLoaded", function (e) {
		...
		$(".card-vendedor-cuenta-pendiente , .card-vendedor").on(
			"click",
			function () {
		...
		$(document).on({
			...
			ajaxError: function (funcion, request, settings) {
				...
				if (request.responseText != "" && request.responseText != undefined) {
					if (
						request.responseText.includes("DELETE") &&
						request.responseText.includes("REFERENCE") &&
						request.responseText.includes("FK")
					) {
						alertify
							.myConfig(
								"Error de Integridad",
								"No se puede eliminar, el registro se encuentra referenciado en otras tablas.",
								function () {
									this.destroy();
									alertify.myConfigAlert(
										"Error de Integridad",
						...
						alertify
							.myConfig(
								"Error",
								`Se ha producido un problema al ejecutar el proceso.
							<br/>
							<br/>
							Para obtener más información de este problema y posibles correcciones, pulse el botón "Ver Detalle" y comuniquese a la línea de servicio al cliente.`,
								function () {
									this.destroy();
									alertify.myConfigAlert(
										"Error",
		...
		$(document).on(
			"keydown",
			"input:not(button, [type=search], .flexdatalist-alias, .chosen-search-input,  .dataTables_filter input), select",
			function (evt) {
				if (evt.keyCode == 13) {
					var fields = $(this)
		...
		$(document).on("click", function () {
			if (
				$(this).closest("[id=liNotificaciones]").length == 0 &&
					...
					success: function (notificaciones) {
						...
						if (notificaciones.length > 0) {
							...
							$.each(notificaciones, function () {
								var notificacion = `<a href="${tipoUrl(
									this["Tipo"],
									this["Numero"]
								)}">
									<div class="panel panel-default panel-noti accionAlerta mb-0 p-0 w-100" data-id="${
										this["AlertaId"]
									}" data-tipo="${this["Tipo"]}" data-descripcion="${
									this["Descripcion"]
								}" data-fecha="${this["Programada"]}">
										<div class="panel-body panel-notificacion noti${
											this["Tipo"]
										} pb-1 pl-2 pr-2 w-100">
		...
		$(document)
			.find("#divDropdownAlerta")
			.on("click", ".accionAlerta", function (e) {
				e.preventDefault();
				if ($(this).attr("data-tipo") == "DE") {
					...
					alertify.confirm(
						"Despertador",
						"Descartar " + descripcion + " <br>Fecha: " + fecha,
						function () {
							$.ajax({
								url: base_url() + "Utilidades/Notificaciones/actualizarAlerta",
								type: "post",
					...
					alertify.confirm(
						"Memos",
						"Alerta de Memo: " + descripcion + " <br>Fecha: " + fecha,
						function () {
							$.ajax({
								url: base_url() + "Utilidades/Notificaciones/actualizarAlerta",
								type: "post",
...
$(document).on("click", "#manualEventos", function (e) {
	e.preventDefault();
	if ($("#modalManualEventos").length === 0) {
		// $.ajax({
		// 	url: `${base_url()}Administrativos/Eventos/ReservarEvento/ManualUsuario`,
		// 	type: "POST",
					...
					<div class="modal-dialog modal-xl" role="document">
						<div class="modal-content" >
							...
							<div class="modal-body" style="height: 70vh;">
								<iframe src="${base_url()}uploads/Manual de usuario COCORA EVENTOS_2.htm" width="100%" height="100%" style="border:none;height:100%;width:100%;"></iframe>
							</div>
						</div>
					</div>
				</div>
