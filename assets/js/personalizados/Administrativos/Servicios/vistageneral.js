...
let accionPos = (VistaGeneralMenu ? 'pedido_mesa' : sessionStorage.getItem('accionPos'));
...
function tipoVenta(infodata) {
	...
	if (($EVENTO > 0 || accionPos == 'general' || accionPos == 'pedido_mesa' || accionPos == 'facturas_pendientes_pago' || accionPos == 'cuentas_pendientes') && infodata.FacturaElectronica == "N" && $DATOSMONTAJE.UVTsFE > 0) {
		if ($DATOSMONTAJE.ValorUVT <= 0) {
			let anio = moment().format("YYYY");
			alertify.alert("Advertencia", `No se ha especificado el valor de la UVT para el año ${anio}. No es posible continuar.`, ()=> { return });
			return;
		}
	}
	...
	if ($EVENTO > 0 || accionPos === 'facturas_pendientes_pago' || accionPos === 'reimprimir' || accionPos === 'informe_vendedor') {
		if (infodata.vendedor == "S") {
			let data = { almacen: idAlmacen, accionPos };
			obtenerInformacion(data, 'obtenerVendedores', 'datosVendedores')
		} else {
			accionVendededor({ vendedorid: null });
		}
	} else {
		...
		if (cargueHotel.ReservaId) {
			urlCargueHotel = "/" + cargueHotel.ReservaId.trim() + "/" + cargueHotel.Tercero.trim();
		}
		location.href = rutaGeneral + 'VistaGeneral/Mesas/' + idAlmacen.trim() + urlCargueHotel;
	}
}
			...
			tiposVenta.forEach(item => {
				$('#estructura').append(`<div class="col-3 p-1">
						<div class="card mb-1 h-100" style="box-shadow: none !important;">
							<div class="card-body p-1" onclick='tipoVenta(${JSON.stringify(item)})' style="border:2px solid #d4d4d4; cursor:pointer; border-radius: 5px; height: 80px;">
								<div class="container-item text-center" style="font-size: 13px; text-overflow: ellipsis; white-space: break-spaces;overflow: hidden;">${item.codiventid} - ${item.nombre}</div>
							</div>
						</div>
					</div>`);
			});
...
function dataUsuario(resp) {
	if (accionPos == 'actualiza_forma_pago' || accionPos == 'reactiva-consumo') {
		...
		if (datosFactura && datosFactura['factura']) {
			abrirCerrarModal('#modal-solicitar-usuario', 'hide', '#modal-numero-factura', 'show');
			setTimeout(() => {
				$("#numFac").focus();
			}, 500);
		} else {
			...
			if (accionPos == 'reactiva-consumo') {
				...
				location.href = base_url() + "Administrativos/Servicios/VistaGeneral?actualiza-forma-pago=true&usuario=" + resp.usuarioId
			}
		}
		...
		location.href = base_url() + "Administrativos/Servicios/InformeGeneralConsumoFacturas?usuario=" + resp.usuarioId;
	} else if (accionPos == 'informe-vendedor-por-consumo') {
		location.href = base_url() + "Administrativos/Servicios/InformeVendedorConsumo?usuario=" + resp.usuarioId;
	} else if (accionPos == 'recogidas' || accionPos == 'cuadreCajero') {
		sessionStorage.setItem('accionPos', accionPos);
		location.href = base_url() + "Administrativos/Servicios/VistaGeneral?" + accionPos + "=true&usuario=" + resp.usuarioId
	} else if (accionPos == 'cambio-almacen-vendedor') {
		location.href = base_url() + "Administrativos/Servicios/CambioVendedor?usuario=" + resp.usuarioId
	} else if (accionPos == 'informe_ventas_cajero') {
		location.href = base_url() + "Administrativos/Servicios/informeVentasCajero"
	}
}
...
function datosResolucion({ datos }) {
	...
	if (datos) {
		if (datos["FE"] == 1 && $POSImprimeComprobanteElec == "N") {
			alertify.alert("Advertencia", "El sistema no esta configurado para imprimir comprobante electronicos", function () {
				$("#formReimprimirFacturas")[0].reset();
				setTimeout(() => {
			...
			Object.keys(datos).forEach(it => $(`#${it}`).val((it != 'FE' ? datos[it] : (datos[it] == 0 ? 'N' : 'S'))));
		}
	}

	$("#prefijo").prop("disabled", false);
	abrirCerrarModal('#modal-solicitar-usuario', 'hide', '#modal-reimprimir', 'show');
	$("#prefijo").unbind().change(function () {
		...
		if (datos['prefijo'] == $(this).val()) {
			...
			obtenerInformacion({ prefijo: $(this).val() }, 'buscarResolucion', 'resolucionBuscada');
		}
	});
	$("#numDocIni").unbind('keyup').keyup(function () {
		let disable = $(this).val() == '' ? true : false;
		$("#numDocFin").prop("disabled", disable);
	});

	$("#numDocIni").unbind('focusout').focusout(function () {
	...
	$("#numDocFin").unbind('focusout').focusout(function () {
		$("#cantidadFacturas").val($("#numDocFin").val() - $("#numDocIni").val()+1);
	});
	$("#formReimprimirFacturas").unbind().submit(function (e) {
		e.preventDefault();
		let $rango = 0;
		...
		if ($("#formReimprimirFacturas").valid()) {
			...
			if ($rango >= 100) {
				alertify.warning("A superado la cantidad máxima de reimpresiones permitidas.")
				return;
			}
			let data = {
				prefijo: $("#prefijo").val(),
...
function facturasObtenidas({ datos, noEncontradas }) {
	...
	alertify.confirm('Advertencia', '¿Desea imprimir los comprobantes de venta?', function () {
		datos.forEach((it) => {
			strReportes += (j == 0 ? '?' : '&');
			...
			if (Object.keys(it.Terceros).length > 1) {
				...
				for (x in it.Terceros) {
					strReportes += '&';
					strReportes += 'r' + j + i + '=ImprimirComprobantePago/' + it.facturaid + '/' + it.Terceros[x] + '/' + i + '/1/1';
					i++;
				}
			}
			j++;
...
function dataFactura({ datos, vendedores }) {
	...
	if (vendedores.length) {
		...
		vendedores.forEach(it => {
			estructura += `<div class="col-12 col-sm-4 col-md-4 col-lg-2 mb-2" style="cursor:pointer">
				<div class="card mb-1 h-100" style="box-shadow: none !important;">
					<div id="vend${it.vendedorid}" class="card-body px-1 py-1 rounded card-vendedor ${it.Actual ? 'card-vendedor-seleccionado' : ''}" style="border:1px solid #b2b9be;" data-vendedor="${it.vendedorid}">
	...
	if (moment(datos.fecha).isSameOrAfter(moment(), 'day')) {
		abrirCerrarModal('#modal-numero-factura', 'hide', '#ElegirVendedor', 'show');
		//location.href2 = rutaGeneral + 'ActualizarFormaPago/factura/' + datosFactura['facturaid'];
	} else {
		alertify.confirm('Alerta', `La factura no pertenece al dia de hoy (${datos.fecha}), ¿Esta seguro que desea editarle el vendedor?`, function (ok) {
			cancelarUsuario();
			abrirCerrarModal('#modal-numero-factura', 'hide', '#modal-solicitar-usuario', 'show');
			setTimeout(() => {
				$("#usuarioid").focus();
			}, 500);
		}, function (err) {
	...
	$(".card-vendedor").click(function () {
		if (datosFactura['vendedorid'] != $(this).data('vendedor')) {
			...
			alertify.confirm('Advertencia', '¿Esta seguro de modificar el vendedor de la factura?', function () {
				let encriptado = $.Encriptar({ vendedor, factura: datosFactura.facturaid, numeroFactura: datosFactura.factura, idAlmacen });
				$.ajax({
					...
					success: function (resp) {
						...
						if (resp.valido) {
							...
							abrirCerrarModal("#ElegirVendedor", 'hide', "#modal-solicitar-usuario", 'hide');
							$("#numFac").val('');
							datosFactura = null;
						} else {
							alertify.error(resp.mensaje);
...
function dataFacturaRactivarConsumo({ datos }) {
	abrirCerrarModal('#modal-numero-factura', 'hide');
	alertify.confirm('Advertencia', `¿Esta seguro de reactivar la ${tipoReactivacion}?`, function () {
...
function redirigirConsumo(resp) {
	...
	sessionStorage.setItem('accionPos', (fac.mesa && fac.mesa != '' ? 'pedido_mesa' : 'general'));
	...
	let datos = {
		...
		, codBarraTercero: resp.infoTercero && resp.infoTercero.barra ? resp.infoTercero.barra : ''
		, numPersonas: fac.personas
		, terceroPedidoEmpresa: JSON.stringify(resp.dataEmpresaFact ? resp.dataEmpresaFact : {})
	}
	sessionStorage.setItem('dataPos', $.Encriptar(datos));
	location.href = base_url() + 'Administrativos/Servicios/VistaGeneral/Mesas/' + fac.AlmacenIdC.trim() + urlCargueHotel;
}
...
function datosVendedores({ vendedores }) {
	...
	if (pos > -1) {
		accionVendededor(vendedores[pos]);
	} else {
		if (vendedores.length) {
			$(".form-buscar-vendedor").addClass('d-flex').removeClass('d-none');
			vendedores.forEach(it => {
				$("#listaVendedores").append(`<div class="col-12 col-sm-4 col-md-4 col-lg-2 mb-2" style="cursor:pointer">
					<div class="card mb-1 h-100" style="box-shadow: none !important;">
						<div id="vend${it.vendedorid}" class="card-body px-1 py-1 rounded card-vendedor" style="border:1px solid #b2b9be;" data-vendedor="${it.vendedorid}">
							<div class="container-vendedor text-center">
							<img src="${it.foto}" style="height: 68px; border-radius: 3px;">
							</div>
							<p style="text-align: center;font-size: 14px;margin-bottom:6px;margin-top: 6px;">${it.nombre}</p>
		...
		$(".card-vendedor").click(function () {
			let actual = this;
			let vend = vendedores.find(it => it.vendedorid == $(this).data('vendedor'));
...
function accionVendededor(vend) {
	if ($EVENTO > 0) {
		location.href = `${base_url()}Administrativos/Eventos/Eventos/IngresoEvento/${idAlmacen}/${$EVENTO}/${vend.vendedorid}`;
	} else if (accionPos === 'informe_vendedor') {
		location.href = rutaGeneral + `InformeVendedor?vendedor=${vend.vendedorid}&almacen=${almacenActual.AlmacenId}`;
	} else if (accionPos === 'reimprimir') {
		...
		location.href = rutaGeneral + `FacturasPendientes?vendedor=${vend.vendedorid}&almacen=${almacenActual.AlmacenId}`;
	}
}
...
function tipoReactivacionEjecutar(tipo, permiso) {
	...
	abrirCerrarModal("#modalTipoReactivacion", 'hide', "#modal-solicitar-usuario", 'show');
}

function reactivarConsumoCortesia() {
	$(".modal-title-reactivar").html('<i style="color:#fff;" class="far fa-image"></i> ' + (tipoReactivacion == 'factura' ? 'Factura' : 'Cortesia'));
	$(".numFacReactivar").html((tipoReactivacion == 'factura' ? 'Prefijo + Número de Factura' : 'Número'));
	abrirCerrarModal("#modal-solicitar-usuario", "hide", '#modal-numero-factura', 'show');
	setTimeout(() => {
		$("#numFac").focus();
	}, 500);
...
function tipoImprecionEjecutar(tipo) {
	if (tipo == 'factura') {
		abrirCerrarModal('#modalTipoImpresion', 'hide', '#modal-reimprimir', 'show');
		$("#formReimprimirFacturas :input").prop("disabled", true);
		obtenerInformacion({idAlmacen}, 'resolucionFacturas', 'datosResolucion');
	} else {
		let title = (tipo == 'comanda_hotel' ? 'Comanda' : 'Cortesia');
		$(".modal-title-impresion").html('<i style="color:#fff;" class="far fa-image"></i> ' + title);
		abrirCerrarModal('#modalTipoImpresion', 'hide', '#modal-numero-imprimir', 'show');
		$("#formNumImpresion").unbind().on('submit', function (e) {
			e.preventDefault();
			if ($(this).valid()) {
				let data = {
					tipoImpresion: tipo,
		...
		$("#btnCancelarNumImpresion").unbind().on('click', function () {
			$('#numImprimir').val('');
...
function datosReimprimir({ datos, tipo, lugarPedido }) {
	if (tipo == 'cortesia') {
		abrirReporte(base_url() + 'Reportes/ImprimirCortesia/' + datos + '/1/1', contexto);
	} else {
		abrirReporte(base_url() + `reportes/imprimirComprobanteConsumoHotel/${datos}/${lugarPedido}/1/1`, contexto);
	}
}
...
$(function () {
	...
	if (cuentaHotel) {
		...
		obtenerInformacion(cargueHotel, 'tipoVentaAlmacenHotel', 'tipoVentaAlmacenH');
	}
	...
	if (((accionPos == 'general') ||
		...
		(accionPos == 'pedido_mesa')) && accionPos != 'cuadreCajero') {
		...
		if ($EVENTO > 0) {
			...
			if ($(this).data('menu') != '') {
				...
				if (!almacenActual.cantZonas) {
					alertify.warning("El almacen no tiene zonas en la sede " + almacenActual.SedeNombre);
					return;
				}
	...
	$("#formNumeroFactura").submit(function (e) {
		e.preventDefault();
		if ($(this).valid()) {
			if (datosFactura && datosFactura['factura'].split(' ').join('').toUpperCase() == $("#numFac").val().split(' ').join('').toUpperCase()) {
				abrirCerrarModal('#modal-numero-factura', 'hide', '#ElegirVendedor', 'show');
				return;
			}
			let data = {
			...
			if (accionPos == 'reactiva-consumo') {
				...
				obtenerInformacion(data, 'validarFactura', 'dataFacturaRactivarConsumo')
			} else {
	...
	//Busqueda del panel princial pal darle enter
	...
	$("#buscarEvento").on("keyup", function () {
		...
		$("#contenedorAlmacen .card-almacen").filter(function () {
			return rex.test($(this).find("h5").text() + " " + $(this).find(".text-muted").text());
		}).removeClass("d-none");
