...
$(function () {
	RastreoIngresoModulo(accionPos == 'general' ? 'Ventas Generales' : 'Pedido Mesa');

	$('.carrito').html($('#content-carrito').html());
	$('#content-carrito').remove();

	//Seteamos los datos para el menú
	if (localStorage.getItem("fontProds") != null && localStorage.getItem("fontProds") != 12) {
		$("#tamLetraProductos").val(localStorage.getItem("fontProds"));
	}

	if (localStorage.getItem("fontPrecioProds") != null && localStorage.getItem("fontPrecioProds") != 14) {
	...
	if (localStorage.getItem("boldLetraProds") != null && localStorage.getItem("boldLetraProds") != 'N') {
		$("#negrillaLetraProductos").val(localStorage.getItem("boldLetraProds"));
	}

	if (localStorage.getItem("boldPrecioProds") != null && localStorage.getItem("boldPrecioProds") != 'S') {
		$("#negrillaLetraPrecio").val(localStorage.getItem("boldPrecioProds"));
	}

	if (localStorage.getItem("disenoProds") != null && localStorage.getItem("disenoProds") != 1) {
	...
	if (dataPos) {
		...
		terceroPedidoEmpresa = dataPos.terceroPedidoEmpresa ? JSON.parse(dataPos.terceroPedidoEmpresa) : {};

		if (dataPos.reactivarConsumo && dataPos.reactivarConsumo != "false") {
			reactivarConsumo = dataPos.reactivarConsumo;
			datosTerceroReactivar = dataPos.terceroDatos ? dataPos.terceroDatos : null;
			...
			if (tipoVentaSeleccionado.manejclien == 'S' && $TIPOCOMERC == 'CLUB') textCambioTercero = "Socio";
			...
			reservaHotel = (dataPos.reservaHotel && dataPos.reservaHotel != 'null') ? dataPos.reservaHotel : null;
			accionTercero = dataPos.AccionPedido ? dataPos.AccionPedido : null;
			habitacionHotel = dataPos.habitacionHotel ? dataPos.habitacionHotel : null;
			dataTerceroPendiente = dataPos.dataTerceroPendiente ? JSON.parse(dataPos.dataTerceroPendiente) : {};
			ventanaCambioPedido = (dataPos.facturaPuntoNoFisico && dataPos.facturaPuntoNoFisico == 'S') ? true : false;
			...
			if (dataPos.consumosOcultos) {
				$(".btnOcultarConsumos").find('i').removeClass('fa-eye').addClass('fa-eye-slash');
		...
		if (accesoCargarCuentaHotel) {
			...
			if (tercDesayun) {
				tercerosDesayunoOriginal = JSON.parse($.Desencriptar(JSON.parse(tercDesayun)));
				tercerosDesayuno = [...tercerosDesayunoOriginal].map(x => x.terceroid);
			}
		}
	}
	...
	if (datosTerceroReactivar && datosTerceroReactivar != '' && typeof datosTerceroReactivar == 'string') {
		try {
	...
	$("#valProducto").change(
		delay(function () {
			if (terceroIdPedido && valorBuscarProducto != $("#valProducto").val().toLowerCase()) {
				...
				if (valorBuscarProducto == '') {
					...
					let grupo = $('.card-tipo-comida.tipo-comida-seleccionado').data('grupo');
					let data = {
						grupoId: grupo,
	...
	if (accionPos == 'pedido_mesa') {
		$("#btnCambiarZona").html('<i class="fas fa-redo-alt"></i> Cambiar De Mesa');
	}

	$("#btnCambiarZona, #tercerosAccion, #btnElegirVendedor, #tituloCentasPendiente, .formBuscarProductos").hide();
	...
	$(document).on('change', "#zonaMesa", function () {
		if ($(this).val() && $(this).val() != '') {
			...
			$("#btnUnificarMesas, #btnCambioMesas, #btnMesasTemporales, #btnEstadoColores").show();
			configColoresMesa();
			...
			if (accionPos != 'general') {
				if ($(this).val() == null) {
					alertify.warning("La zona predeterminada del almacen es de una sede diferente.");
	...
	if (($INFOALMACEN['ManejaMesas'] != 'S' && $ZONAS.length) || accionPos == 'general') {
		if (accionPos == 'general') {
			if ($POST) {
	...
	$("#btnCancelarVendedor").click(function () {
		if (accionPos == 'general') {
			...
			if (!terceroIdPedido) {
				abrirCerrarModal("#ElegirVendedor", "hide", ".bd-accion-modal-sm", "show");
			} else {
	...
	$("#formCargarAccion").submit(function (e) {
		...
		if ($(this).valid()) {
			if (cambiarSocioCuenta) {
				let enc = tercerosAccion.find(it => it.TerceroID == $("#UserPedido").val());
				...
				let datos = {
					...
					personas: $("#NumeroPersonas").val() == '' ? 1 : $("#NumeroPersonas").val(),
					terceroEmpresa: terceroPedidoEmpresa.TerceroID ? ("" + terceroPedidoEmpresa.TerceroID).trim() : null
				}
				if (enc.TipoDocumento != '31' || tipoVentaSeleccionado.manejclien == 'N') {
				...
				$.ajax({
					...
					success: (resp) => {
						...
						if (resp.valido) {
							...
							let maxPer = $(".mesaId.mesa-seleccionada").data('personas') ? $(".mesaId.mesa-seleccionada").data('personas') : null;
							$("#cantPersonas").html(`<input type="number" name="NumPerEditar" style="color: #ffffff;" class="form-control form-control-floating" placeholder="Número de personas" id="NumPerEditar" data-campodb="Personas" value="${numeroPersonasPedido}" max="${maxPer}" min="1" required>
							`);
							...
							$("#btnCuentaHotel").hide();
							reservaHotel = null;
							if (cargarCuentaHotelPermiso && enc.ReservaHotel && enc.ReservaHotel != -1) {
				...
				if (enc) {
					...
					if ($DATOSMONTAJE.FactConAbier == 'S' && enc.TotalEdicion > 0) {
						alertify.error(`${enc.NombreTercero} se encuentra con un pedido en edición por el usuario ${enc.NombreUsuarioEdicion}`);
						return
					}
					...
					if (datosCuentaHotel) {
						datosCuentaHotel = JSON.parse($.Desencriptar(JSON.parse(datosCuentaHotel)));
						enc.AccionId = datosCuentaHotel.AccionId ? datosCuentaHotel.AccionId : null;
					...
					if (btnAgregarCuentaTercero && terceroIdPedido) {
						obtenerInformacion({ mesaId: idMesaActual, tercero: terceroIdPedido }, 'EdicionMesa');
					}
					...
					HeadReservaIdHotel = enc.HeadReservaHotel;
					if (accesoCargarCuentaHotel && terceroIdPedido != enc.TerceroID.trim()) {
						alertify.warning("El tercero elegido es diferente al de la reserva hotel.");
						return
					}
					if ($TIPOCOMERC == 'CLUB' && enc.firma != 'S') {
						alertify.confirm(
	...
	$("#formIngredientes").submit(function (e) {
		...
		if ($(this).valid()) {
			...
			if (+$("input[name=Cantidad]").val() <= 0) {
				return alertify.warning("No es posible agregar productos con cantidad 0.")
			}
			...
			let observsValidas = productoActual.inventames != 'S' && productoActual.estructura == 'S' && itemsObservaciones.length > 0;

			/* Se valida para cuando un prod de familia maneja terminos */
			if (productoActual.AplicaFamilia == 'S' && productoActual.TotalFamilias > 0 && !productoActual.SeleccionoFamilia) {
				productoActual.valorTermino = data.Termino;
			}

			if (productoActual.AplicaFamilia == 'S' && productoActual.TotalFamilias > 0 && !productoActual.SeleccionoFamilia && !observsValidas) {
				let dataFami = {
					producto: productoActual.headprodid
				}
				obtenerInformacion(dataFami, 'obtenerFamiliasProd', 'informacionFamiliaProducto');
			} else {
				...
				if (valorProd <= 0 && $DATOSMONTAJE['ventaceros'] != 'S' && tipoVentaSeleccionado.PreciAbier == 'S') {
					alertify.error("No se especificó el valor unitario del producto o no es valido");
					return;
				}

				if (promocionActual['Tipo'] == 'RANC') {
					if (!validarPromociones(promocionActual, promocionActual)) {
						alertify.warning("La cantidad ingresada no aplica para la promoción de rango.")
					}
				}
				...
				if (itemsNoDisponiblesProducto.length) {
					...
					itemsNoDisponiblesProducto.forEach(op => {
						let enc = data.inventario.find(it => op.productoid == it.productoid);
						if (!enc) {
							datos.push(enc);
						}
					});
				...
				let info = {
					...
					, terceroEmpresa: terceroPedidoEmpresa['TerceroID'] ? terceroPedidoEmpresa['TerceroID'] : terceroIdPedido
				...
				if (productoActual.EsDesayuno == 'S' && ReservaAplicaDesayuno == 'S' && TieneDesayunoPendiente == 'S') {
					if (info.Cantidad > 1) {
						return alertify.warning(`El tercero tiene solo un desayuno pendiente`);
					}
				}
				if (editarProductoPedidoNuevo > -1) {
					...
					if (tercerosAccion && tercerosAccion.length) {
						let value = tercerosAccion.find(op => op.TerceroID == terceroIdPedido);
						if (value) {
				...
				if (productoActual.PromoDatosProdObsequio && productoActual.PromoDatosProdObsequio['headprodid']) {
					...
					if (validAdd && tempProdAg.inventames == "N" && tempProdAg.estructura == "S") {
						if (!validarEstructuraHijos(tempProdAg.inventario, cantValid)) {
							validAdd = false;
						}
					} else {
						if (validAdd && tempProdAg.inventames == "S" && +tempProdAg.invenactua < cantValid) {
							validAdd = false;
						}
					}
					if (!validAdd) {
						alertify.warning("Se retirará el producto de obsequio ya que no se encuenta con inventario disponible.");
					} else {
				...
				if (productoActual.AplicaFamilia == 'S' && productoActual.TotalFamilias > 0 && totalFami > 0) {
					...
					if (editarProductoPedidoNuevo > -1) {
						prodsSelectFamilia[posPlatoActualFami] = prodsSelectFamilia[posPlatoActualFami].map(it => {
							it.Cantidad = it.CantidadFamilia;
							it.allevar = arrayProductosPedido[editarProductoPedidoNuevo].allevar;
							it.Desayuno = arrayProductosPedido[editarProductoPedidoNuevo].Desayuno;
							it.Personas = arrayProductosPedido[editarProductoPedidoNuevo].Personas;
							it.Termino = arrayProductosPedido[editarProductoPedidoNuevo].Termino;
							it.PuestoMesa = arrayProductosPedido[editarProductoPedidoNuevo].PuestoMesa;
							it.Valor = 0;
							it.nuevo = true;
							it.Observacio = "";
						...
						arrayProductosPedido[editarProductoPedidoNuevo].ProductosFamilia = prodsSelectFamilia[posPlatoActualFami];
						editarProductoPedidoNuevo = -1;
					} else {
						if (itemsObservaciones.length) {
							info.inventario = info.inventario.concat(productoActual.observacionesProd);
						}
				...
				if (observsValidas) {
					...
					$("#valProducto").val('');
					promocionActual = {};
					if (editarProductoPedidoNuevo != -1 && arrayProductosPedido[editarProductoPedidoNuevo]['Id'] && arrayProductosPedido[editarProductoPedidoNuevo]['Imprime'] != "P") {
						permisoAccionActual = 2609;
						imprimirDevolucionConsumo = false;
						if ($DATOSMONTAJE.TiempoDevolConsumo && productoActual.Cantidad > data.Cantidad) {
							let tiempoDevolucion = moment(productoActual.FechaConsu).add($DATOSMONTAJE.TiempoDevolConsumo, 'minutes');
	...
	$("#formObservacionesProd").submit(function (e) {
		...
		if ($(this).valid()) {
			...
			if (editarProductoPedidoNuevo != -1 && arrayProductosPedido[editarProductoPedidoNuevo]['Id'] && arrayProductosPedido[editarProductoPedidoNuevo]['Imprime'] != "P") {
				permisoAccionActual = 2609;
				imprimirDevolucionConsumo = false;
				if ($DATOSMONTAJE.TiempoDevolConsumo && productoActual.Cantidad > cantidadNuevaPedido) {
					let tiempoDevolucion = moment(productoActual.FechaConsu).add($DATOSMONTAJE.TiempoDevolConsumo, 'minutes');
					if (tiempoDevolucion.isBefore(moment())) {
						abrirCerrarModal("#modal-solicitar-usuario", "show");
					}
	...
	$("#btnCuentaHotel").click(function () {
		...
		if (accesoCargarCuentaHotel) {
			...
			if (tipoVentaSeleccionado.Propina == "S") {
				alertify.propinaAlert($('#propinaFrm')[0], function () { }).set('selector', 'input[id="propinaInput"]');
			} else {
				guardarCuenta();
			}
		} else {
	...
	$("#btnFacturarPedido").click(function () {
		...
		if ($DATOSMONTAJE['FacturaCortesia'] == 'S' && $DATOSMONTAJE['ventaceros'] == 'S' && +valor <= 0) {
		...
		if ($DATOSMONTAJE['ConfirmarConsumoAPP'] == 'S') {
			let SinConfirmar = arrayProductosPedido.filter(ite => !ite.ConfirmadoAPP);
			if (SinConfirmar.length) {
				alertify.warning("Algunos productos no han sido aceptados por el cliente.");
				return;
			}
		}
		...
		if ($DATOSMONTAJE.FactConAbier == 'S') {
			...
			if ($(".btnOcultarConsumos").hasClass('ocultarConsumos')) {
				...
				let printConsumos = arrayProductosPedido.filter(it => {
					return $(`.platos-pedido-item[data-prod=${it.ProductoId}]`).hasClass('activoplato')
				}).map(x => x.Id);
	...
	$("#tabProductos").click(function () {
		if (!$('.card-tipo-comida').children().children('.producto-seleccionado')[0]) {
	...
	$("#formDataAdmin").submit(function (e) {
		...
		if ($(this).valid()) {
			...
			if (permisoAccionActual == 2609) {
				...
				obtenerInformacion(data, 'validarUsuario', "borrarCuentaProductoPedido");
			}
		} else {
			alertify.error("Validar la información de los campos");
		}
	...
	$("#btnCerrarModalAccionesCargarCuenta").click(function () {
		abrirCerrarModal("#modal-accion-tercero", "hide", ".bd-accion-modal-sm", "show");
	});
	...
	$('.card-vendedor').on('click', function () {
		...
		if ($DATOSMONTAJE.SolicitaClaveVendedor == 'S' && $(this).data("venuser") == '') {
			let mensaje = `
	...
	$("#UserPedido").change(function () {
		...
		if (enc) {
			...
			dataMostrarTercero.forEach(it => {
				...
				<div class="col-6 prop${it.propiedad}">${(enc[it.propiedad] == null ? '' : enc[it.propiedad])}</div>
			`);
			});
			...
			if (enc.EsEmpleado == 0 && !enc.Ingreso && $INFOALMACEN['VerificarIngreso'] == 'S' && (!enc.ReservaHotel || enc.ReservaHotel == -1) && enc.TipoDocumento != '31') {
			...
			if (enc.CantAcciones > 1 && $(".btnFiltroeBuscar.show").data('botonsearch') == 'habitacion') {
			...
			} else if (enc.CantAcciones > 1 && columnaBusquedaTercero != 'AccionId' && !enc['IdReserva']) {
				...
				if (enc.TipoDocumento == '31') {
					if (!cambiarSocioCuenta) {
						if (!terceroPedidoEmpresa.TerceroID) {
							...
							$('.empresa-datos').html(`<strong>Empresa: </strong> ${enc.Nombre} - <strong> Nit: </strong> ${enc.TerceroID} - <strong> Acción: </strong> ${enc.AccionId || ''}`);
						...
						if (accionTercero && accionTercero != '') {
							if (enc.Acciones) {
								let acci = enc.Acciones.find(it => it.AccionId == accionTercero);
	...
	$("#mesasUnificar").change(function (e) {
		let entrada = $(this).val().filter(x => !selectMesasMultiple.includes(x));
		if (entrada[0] == "") {
			$(this).val([""]);
		} else {
		...
		if (unificarMesasTercero) {
			...
			if (!selectMesasMultiple.length || selectMesasMultiple[0] == "") {
				...
				$(`a.link-mesas[data-mesaterc=${mesabtn}]`).removeClass('mesa-unificar');
			} else {
	...
	$("#btnOtrasCuentasTerceroAceptar").click(function () {
		...
		if (mesas.length) {
			...
			obtenerInformacion(data, 'unificarMesasTercero', 'mesasUnificadasTercero')
		} else {
	...
	$("#btnCambiarZona").on('click', function () {
		if (accionPos == 'pedido_mesa' && $INFOALMACEN['ManejaMesas'] == 'S') {
			...
			$('.card-vendedor').removeClass('card-vendedor-seleccionado');
			$("#dataTercero").html("");
			$(".btnOcultarConsumos").find('i').removeClass('fa-eye-slash').addClass('fa-eye');
	...
	$("#formObservacionBorrar").submit(function (e) {
		...
		if ($(this).valid()) {
			if (permisoAccionActual == 2609) {
				arrayProductosPedido[editarProductoPedidoNuevo]['ObservacioDevol'] = $("#ObservacioBorrar").val();
				editarProductoPedidoNuevo = -1;
			...
			let data = {
				...
				tercero: terceroIdPedido,
				mesa: idMesaActual,
				tipoDevo: tipoDevolucionSeleccionada && tipoDevolucionSeleccionada.tipodevoid || null,
				accion: accionTercero,
				UsuarioIdAprueba: (usuarioIdA === 'undefined' || usuarioIdA == null) ? null : usuarioIdA,
			};
			if (formValidateDataUser === 'producto') {
				data['Id'] = arrayProductosPedido[editarProductoPedidoNuevo]['Id'];
				data['Producto'] = arrayProductosPedido[editarProductoPedidoNuevo]['ProductoId'];
			}
			$.ajax({
				url: rutaGeneral + 'borrarDeCuenta',
				type: 'POST',
				data: { encriptado: $.Encriptar(data), imprimirDevolucionConsumo, accionPos },
				dataType: "json",
				success: (resp) => {
					resp = JSON.parse($.Desencriptar(resp));
					if (resp.valido) {
						...
						$("#ObservacioBorrar").val('');
						if (formValidateDataUser === 'producto') {
							if (!arrayProductosPedido[editarProductoPedidoNuevo]['pendiente'] && arrayProductosPedido[editarProductoPedidoNuevo]['Desayuno'] == 'S') {
	...
	$(".btnCancelarObservacionBorrar, #btnCancelarTipoDevolucion").click(function () {
		tipoDevolucionSeleccionada = null;
		editarProductoPedidoNuevo = -1;
		prodsAgregarNew = [];
	...
	$("#btnCargarMesasTemporales").on('click', function () {
		...
		abrirCerrarModal(".modal-mesas-activar", "hide", "#modal-solicitar-usuario", "show");
	});
	...
	$('.bd-accion-modal-sm').on('shown.bs.modal', function (event) {
		...
		if (accionPos == 'pedido_mesa') {
			...
			if (mesa.TotalPendientes > 0) {
				...
				$("#btnVerCuentasPendientes").unbind().click(function () {
					let data = {
						mesa: idMesaActual,
						fechaActual: moment().format('YYYY-MM-DD HH:mm:ss'),
						zona: (zonaMesaActual['ZonaId'] || '')
					}
					obtenerInformacion(data, 'cuentasConsumoPendientes', 'consumosPendientesMesa')
				});
			}
			setTimeout(() => {
			...
			if (accionPos == 'general') {
				if (terceroIdPedido == "" && tercerosDesayuno.length) {
					...
					let data = {
						...
						, terceroEmpresa: terceroPedidoEmpresa && terceroPedidoEmpresa.TerceroID ? ("" + terceroPedidoEmpresa.TerceroID).trim() : null
					}
	...
	$("#btnDarDeBajaCuenta").click(function () {
		if (!darBajaVenta) {
			...
			$(".platos-pedido-item").unbind().click(function () {
				if ($(this).data('valido') == 'S' && darBajaVenta && $(this).data("consumoid") != 0) {
			...
			$("#btnCancelarProceso").unbind().click(function () {
				darBajaVenta = false;
				$(".platos-pedido-item button").attr('disabled', false);
				$(".botones-carrito button").css('pointer-events', 'auto');
				$(".platos-pedido-item").removeClass('dar-baja-producto').off('click');
				$(this).hide();
			});
		} else {
	...
	if ($DATOSMONTAJE['ConfirmarConsumoAPP'] == 'S') {
		$("#btnConfirmarConsumo").click(function () {
			alertify.prompt('Contraseña', $(".clientePedido:first").text(), '', function (evt, value) {
				if (value != '') {
					let info = {
						tercero: terceroIdPedido,
						pass: value
					}
					obtenerInformacion(info, 'validarPassSocio', 'socioValidoConfirmarConsumo');
				} else {
					setTimeout(() => {
	...
	if (!accesoCargarCuentaHotel && accionPos == 'pedido_mesa') {
		$("#btnCargarDesayunos").click(function () {
			sessionStorage.setItem('regresoMesaDesayuno', $.Encriptar($AlmacenId));
		...
		$("#btn-cancelar-pedido").click(function () {
			if (tercerosDesayuno.length > 0) {
				...
				$(document).find(".platos-pedido").find(".platos-pedido-item").each(function () {
					let plato = $(this).data();
	...
	$("#btnPrecuenta").on('click', function () {
		...
		let printConsumos = arrayProductosPedido.filter(it => {
			return it.ConsumoId > 0 && $(`.platos-pedido-item[data-prod=${it.ProductoId}]`).hasClass('activoplato')
		}).map(x => x.Id).join('-');
		if (printConsumos.length) {
			abrirReporte(`${base_url()}reportes/imprimirCuenta/${printConsumos}/${tipoVentaSeleccionado.codiventid}/${tipoVentaSeleccionado.impresion}`);
		} else {
		...
		if (tipoVentaSeleccionado.vendedor && tipoVentaSeleccionado.vendedor == 'S') {
			if (vendedorId == 0) {
				...
				if (venUser) {
					...
					$("#vend" + $VENDEDORES[0]['vendedorid']).addClass('card-vendedor-seleccionado');
					$("#btnFacturarFacturaElectronico").attr('data-vendfactura', 'S');
					if ($VENDEDORES[0]['PuedeFacturarEnPOS'] != 'S') {
						$("#btnFacturarPedido").prop('disabled', true);
						$("#btnFacturarFacturaElectronico").attr('data-vendfactura', 'N');
					}
					abrirCerrarModal(".bd-accion-modal-sm", "hide", "#ElegirVendedor", "show");
					validarBtnFacturaElectronica();
				} else {
					setTimeout(() => {
						abrirCerrarModal(".bd-accion-modal-sm", "hide", "#ElegirVendedor", "show");
...
function cuentaPendiente({ consumo }) {
	...
	if (consumo.length) {
		...
		$(".btnTerceroCuentaPendiente").on('click', function () {
			...
			if ($TIPOCOMERC == 'CLUB' && $(this).data('zona') != null) {
				alertify.warning("El tercero tiene cuenta en las zonas: " + $(this).data('zona'));
			}
			if ($TIPOCOMERC == 'CLUB' && $(this).data('mesa') != null) {
				alertify.warning("El tercero tiene cuentas en las mesas: " + $(this).data('mesa'));
			}
			...
			if ($TIPOCOMERC == 'CLUB' && $(this).data('firma') != 'S') {
				...
				alertify.confirm(
					...
					, function (evt, value) {
						...
						if (canAcciones.length > 1) {
							if (cuentaTerceroConsumo.CantAcciones > 1 && !cuentaTerceroConsumo.IdReserva) {
								let acciones = [];
								canAcciones.forEach(op => {
									let enc = cuentaTerceroConsumo.Acciones.find(it => it.AccionId.trim() == op.trim());
				...
				accionTercero = (!canAcciones[0] || canAcciones[0] == '' ? null : canAcciones[0]);
	...
	if (mesa['TerceroReserva']) {
		...
		if (mesa['EstadoReserva'] == 'EM') {
			...
			let fecha = moment(reservaActual['Inicio']).subtract(zonaMesaActual['TiempoAntesReserva'], 'minutes');
			let fecha2 = moment(reservaActual['Inicio']).add(zonaMesaActual['TiempoEsperaReserva'], 'minutes');
			if (mesa.Consumo > 0) {
				$("#btnCargarAccion").hide();
			} else if (fecha.isBefore(moment()) && moment(reservaActual['Inicio']).isAfter(moment())) {
				alertify.warning(`La mesa ${mesa['MesaId']} tiene una reserva a las ${reservaActual['Hora']}.`);
				return;
			} else if (moment().isBetween(moment(reservaActual['Inicio']), fecha2)) {
				alertify.warning(`La mesa ${mesa['MesaId']} tiene una reserva pendiente.`);
				return;
			}
		}
	}
...
function seleccionTercero($this) {
	...
	if ($this.data('mesa') == null) {
		...
		let mesasFun = (typeof mesitas == "string" ? mesitas.split(' ').join('').split('-') : [mesitas + ""]);
		let nombre = $this.data('nombre');
		if ($TIPOCOMERC == 'CLUB' && accionPos == "pedido_mesa") {
			if ($DATOSMONTAJE.FactConAbier == 'S') {
				...
				setTimeout(() => {
					alertify.confirm('Alerta', `El tercero tiene cuentas en la(s) mesa(s) ${$this.data('mesa')}, ¿Desea unificar con la mesa ${idMesaActual}?`, function (ok) {
						$(".clientePedido").html(nombre);
						$(".accionIdPedido").html((accionTercero || ''));
						let data = {
						...
						obtenerInformacion(data, 'unificarMesasTercero', 'validarVendedor')
					}, function (err) {
						$("#zonaMesa").change();
						terceroIdPedido = null;
						accionTercero = null;
...
function agregarMesas({ datos, reservas }) {
	$('.contenedor-imagen-mesas').css('background-image', ("url('" + zonaMesaActual.Imagen + "')"));
	$('.divMesa').empty();
	mesasPlano = datos;
		...
		setTimeout(() => {
			if (cambiarMesas || unificarMesas) {
				...
				if (!mesaUno) {
					if (mesa.Consumo > 0) {
						mesaUno = mesa;
						let mensaje = (cambiarMesas ? 'Seleccione la nueva mesa' : 'Seleccione la mesa con la cual desea unificar');
				...
				if (mesaUno && mesaDos) {
					...
					if (cambiarMesas) {
						...
						obtenerInformacion({ mesa: mesaUno['MesaId'], cambio: true }, 'informacionUnificarCambioMesas', 'infoCambioMesas');
					} else {
						/* Codigo para unificarMesas del plano */
						if (mesaUno.Consumo > 1) {
							obtenerInformacion({ mesa: mesaUno['MesaId'] }, 'informacionUnificarCambioMesas', 'infoUnificarMesas');
						} else {
							alertify.confirm('Alerta', `¿Esta seguro de pasar la mesa ${mesaUno['MesaId']} a la mesa ${mesaDos['MesaId']}?`, function (ok) {
								let data = {
									mesaSalida: mesaUno['MesaId'],
									mesaEntrada: mesaDos['MesaId']
								}
				...
				$('.card-tipo-comida').children().children('div').removeClass('producto-seleccionado');
				$("input[name=Termino]").removeAttr("required");
				...
				$("#NumeroPersonas").attr('max', $(".mesaId.mesa-seleccionada").data('personas'));
	...
	if (reservas.length) {
		...
		$("#lista-reservas").html('<div class="text-center">No hay reservas disponibles</div>');
		$("#lista-reservas-superadas").html('<div class="text-center">No hay reservas disponibles</div>');
	}
}
...
$(document).on("click", "#btnverMesa", function () {
	abrirPlano(base_url() + 'Plano/mesas/' + Nitmesa + '/' + $INFOALMACEN.almacenid.trim() + '/' + $INFOALMACEN.ZonaMesaId);
});
...
function infoUnificarMesas({ datos }) {
	...
	datos.forEach((it, index) => {
		...
		$("#listaTercerosConsumo").append(`<div class="text-left col-12 px-0 d-flex justify-content-between mt-2 align-items-center">
			<p class="m-0 font-weight-600 w-100">
				<span class="text-primary">${it.nombre}</span>
				${it.AccionId ? `<span class="text-primary" title="Accion"> - ${it.AccionId}</span>` : ''}
			</p>
			<div class="form-group">
				<div class="custom-control custom-switch">
					<input type="checkbox" class="custom-control-input" data-user="${it.AccionId}" data-infomesa="${tercero}" name="Est${tercero + ("" + index)}" id="Est${tercero + ("" + index)}">
		...
		it.productos.forEach(op => {
			...
			productos += `<div class="col-3 p-1">
				<div class="card mb-1 h-100" style="box-shadow: none !important;">
					<div class="card-body p-1 card-comida-cambio" data-checkpadre="${tercero + index}" data-consumo="${op.Id}" id="${op.productoid + op.Id}" data-check="false" style="border:2px solid #b2b9be; cursor:pointer; border-radius: 5px;">
						<div class="container-item text-center" style="font-size: 12px; text-overflow: ellipsis; white-space: break-spaces;overflow: hidden;">${op.nombre}</div>
					</div>
				</div>
			</div>`;
		});
		$("#listaTercerosConsumo").append(`<div class="text-left col-12 px-0 mt-2 align-items-center">
			<div class="d-flex justify-content-between">
				...
				<div class="form-group">
					<div class="custom-control custom-switch">
						<input type="checkbox" class="custom-control-input checkpadre" data-terc="${tercero}" data-consu='${JSON.stringify(ids)}' name="${tercero + index}" id="${tercero + index}">
	...
	copiaDatos.forEach(op => {
		...
		(op.inventario || []).forEach(inv => {
			if (inv.Tipo && inv.Tipo != ' ') {
				...
				if ((inv.Tipo == 'F' && !inv.elegido) || (inv.Tipo == 'V' && inv.elegido) || (inv.Tipo == 'O' && inv.elegido)) {
					tipos[tipo] = (tipos[tipo] ? tipos[tipo] + ', ' : ' _' + tipo + '_:') + ' ' + inv.nombre;
				}
			}
		});
		if (op.Observacio != "") {
			op.Observacio = op.Observacio.includes('Sin:') || op.Observacio.includes('V:') || op.Observacio.includes('O:') ? "" : op.Observacio;
		}
		Object.keys(tipos).forEach(it => op.Observacio += tipos[it]);

		// op.Observacio += (op.Termino ? ' T: ' + op.Termino : '');
		...
		let Valor = ((op.valorUnitario ? +op.valorUnitario : +op.Valor) * op.Cantidad);
		...
		let datos = {
			...
			FechaConsu: (op.FechaConsu ? moment(op.FechaConsu).format('YYYY-MM-DD HH:mm:ss') : null),
			...
			ReservaHotel: (accesoCargarCuentaHotel && op.ReservaHotel ? op.ReservaHotel : (!reservaHotel || reservaHotel == -1 || filterNuevo ? null : reservaHotel)),
			Termino: (op.Termino ? op.Termino : ''),
			TerceroId: op.terceroEmpresa && op.terceroEmpresa != '0' ? (op.terceroEmpresa + '').trim() : tercero,
			TerceroIdC: tercero,
			TipoCartId: op.TipoCarteraId,
			tipodevol: (op.tipodevol ? op.tipodevol : null),
			tipoVenta: (tipoVentaSeleccionado.CodigEstru || null),
			VendedorId: (op.nuevo ? (vendedorElegido || null) : (op.VendedorId ? op.VendedorId : null)),
			HabitacionId: (accesoCargarCuentaHotel && op.ReservaHotel
				? habitacionTemp
		...
		if (op.Id) {
			...
			if (!(array[0] && array[0]['Imprime'] == 'S' && array[0]['Despachado'] == 'S' && array[0]['Entregado'] == 1 && array[0]['HoraDespacho'] == null)) {
				datos['Imprime'] = '';
				datos['Despachado'] = '';
		...
		if (op.AplicaFamilia = "S" && op.ProductosFamilia && op.ProductosFamilia.length) {
			...
			datosGuardar = datosGuardar.concat(organizarDataConsumo(op.ProductosFamilia, tercero, filterNuevo));
		}
	});
	return datosGuardar;
}
...
function clickMesa({ datos, consumo, montaje, subgrupo }) {
	...
	if (localStorage.getItem("boldLetraProds") != null && localStorage.getItem("boldLetraProds") != 'N') {
		fontProds += localStorage.getItem("boldLetraProds") == "S" ? " font-weight: bold;" : "";
	}

	if (subgrupo == 'N') {
		$(".linea-subgrupos").hide();
		datos.forEach(op => {
			$("#lista-tipo-comidas").append(`<div class="col-4 col-md-3 col-sm-4 col-lg-2 col-xl-2 card-tipo-comida px-1 mb-2" data-grupo="${op.GrupoId}" style="cursor:pointer; height: ${baseHeight}px; max-height: ${baseHeight}px;" title="${op.Nombre}">
				<div class="card mb-1 h-100" style="box-shadow: none !important;">
					<div class="card-body px-1 py-1" style="border:1px solid #b2b9be;">
						<div class="d-flex flex-column align-items-center justify-content-center h-100" ${!op.Imagen ? `style="background-color: #${op.Color};"` : ''}>
							${op.Imagen ? `<img src="${op.Imagen}" style="width:auto; height: 75%; border-radius: 3px;max-width: 100%;">` : ''}
							<p class="fontProds text-center ${op.Imagen ? 'text-truncate m-0' : 'm-2'}" style="${op.Imagen ? 'width: 95%; height: 25%;' : ''} ${fontProds}">${op.Nombre}</p>
						</div>
		...
		if (!$POST) {
			if (permisoAccionActual == 2594) {
				arrayProductosPedido = consumo.concat(arrayProductosPedido.filter(it => it.nuevo && !it.pendiente));
			} else {
			...
			$POST = false;
			if (consumo.length) {
				numeroPersonasPedido = (consumo[consumo.length - 1].Personas ? consumo[consumo.length - 1].Personas : 1);
			} else {
				numeroPersonasPedido = 1;
			}
			arrayProductosPedido = consumo;
		...
		if (arrayProductosPedido.length > 0 && consumo.length > 0) {
			...
			let maxPer = $(".mesaId.mesa-seleccionada").data('personas') ? $(".mesaId.mesa-seleccionada").data('personas') : null;
			$("#cantPersonas").html(`<input type="number" name="NumPerEditar" class="form-control form-control-floating" style="color: #ffffff;" placeholder="Número de personas" id="NumPerEditar" data-campodb="Personas" value="${numeroPersonasPedido}" max="${maxPer}" min="1" required>
			`);
			accionCamposPedido();
		} else {
			$(".totalCuentaActual").text('$0.00');
		...
		$(".formBuscarProductos").show();
		abrirCerrarModal(".modal-cuentas-pendientes", "hide");
		if (!desdeFactura && (!datosTerceroReactivar || (datosTerceroReactivar && !datosTerceroReactivar.TerceroID))) {
...
function organizarSubGruposLineaTiempo(eliminar = -1) {
	...
	lineaTiempoSubGrupos.forEach((it, pos) => {
		estructura += `<li class="breadcrumb-item item-subgrupo" data-grupo="${it.GrupoId}" data-pos="${(pos + 1)}">
			<a href="#">${it.Nombre}</a>
		</li>`;
	});
	$(".linea-subgrupos").html(`<ol class="breadcrumb mb-1">${estructura}</ol>`).show();
...
function accionCamposPedido() {
	$("#NumPerEditar").unbind().change(function () {
		if ($("#formPedido").valid()) {
			let data = {
				tercero: terceroIdPedido,
	...
	if (dataMesas.length) {
		...
		dataMesas.forEach((item, pos) => {
			tabs += `<li class="nav-item text-center" role="presentation">
				<a class="nav-link text-truncate ${pos == 0 ? 'active' : ''} link-mesas" data-mesaterc="${item['mesa']}" title="M${item['mesa']}" id="M${item['mesa']}-tab" data-toggle="tab" href="#M${item['mesa']}" role="tab" aria-controls="M${item['mesa']}" aria-selected="true">${item['mesa']}</a>
			</li>`;
			let contentTab = `
			...
			item.productos.forEach((option, pos2) => {
				contentTab += `<div title="${toTitleCase(option.nombre)}" class="list-group-item list-group-item-active p-2" style="border-top-width: 1px;">
		...
		setTimeout(() => {
			...
			$("#btnUnificarMesasTercero").html("Unificar a la mesa " + dataMesas[0]['mesa']);
			$("#btnUnificarMesasTercero").attr('data-mesabtn', dataMesas[0]['mesa']);
			accionMesasTercero();
		}, 200);
	} else {
		$("#contenidoModalCuentatercero").html('<div class="text-center">No hay cuentas disponibles</di>');
		$("#content-select").hide();
	}
...
function accionMesasTercero() {
	$(".link-mesas").click(function () {
		if (unificarMesasTercero) {
			if ($(this).data('mesaterc') != $("#btnUnificarMesasTercero").data('mesabtn')) {
			...
			$("#btnUnificarMesasTercero").html("Unificar a la mesa " + $(this).data('mesaterc'));
			$("#btnUnificarMesasTercero").attr('data-mesabtn', $(this).data('mesaterc'));
		}
	});
}
...
function cambiarTab(elemento) {
	...
	if (elemento == '#tabCategorias' && accionPos == 'pedido_mesa') {
		$("#lista-menu, .breadcrumb-item:eq(0), .main-menu-header").addClass("disabled-events");
		$("#sincronizarPermisos, #btnTecladoHUD").addClass("disabled-events disabled-event");
		$("#nav-user-link").find(".list-inline-item:eq(0)").addClass("disabled-events");
		$(".pro-body").find("li:eq(0)").addClass("disabled-events");
		$(".b-brand").addClass("disabled-event");
	} else {
		$("#lista-menu, .breadcrumb-item:eq(0), .main-menu-header").removeClass("disabled-events");
		$("#sincronizarPermisos, #btnTecladoHUD").removeClass("disabled-events disabled-event");
		$("#nav-user-link").find(".list-inline-item:eq(0)").removeClass("disabled-events");
...
function consumosPendientesMesa({ datos }) {
	...
	if (datos.length) {
		datos.forEach(it => {
			estructura += `<div title="Ir a la cuenta" class="m-b-10 d-flex border rounded btnTerceroConsumoPendiente" style="align-items: center;" data-info="${it.TerceroIdC}" data-accion="${it.Accion || ''}" data-nombre="${it.nombre}" data-personas="${(it.PersonasReserva ? it.PersonasReserva : it.Personas)}" data-firma="${it.firma}">
				<img src="${it.foto}" style="width: 70px; height: auto; max-height: 75px; border-radius: 3px" class="mr-3">
				<div style="width: 72%;">
					<h6>${it.nombre}</h6>
		...
		$("#cuentasPendientesConsumo").html('<div class="text-center">No hay consumos pendientes</div>');
	}
	abrirCerrarModal(".bd-accion-modal-sm", "hide", ".modal-cuentas-pendientes", "show");
	$(".btnTerceroConsumoPendiente").unbind().click(function () {
		terceroIdPedido = $(this).data('info');
		...
		$("#cantPersonas").html(`<input type="number" name="NumPerEditar" style="color: #ffffff;" class="form-control form-control-floating" 	placeholder="Número de personas" id="NumPerEditar" data-campodb="Personas" value="${numeroPersonasPedido}" max="${maxPer}" min="1">
		`);
		if ($TIPOCOMERC == 'CLUB' && $(this).data('firma') != 'S') {
			alertify.confirm(
				'Alerta',
...
function clickTipoMenu({ datos }) {
	...
	datos.forEach(op => {
		...
		let productoValido = (op.veriInven == 'SI' || op.AplicaSubGrupo > 0 ? true : false);
		...
		fontPrecioProds = "";
		if (localStorage.getItem("fontPrecioProds") != null && localStorage.getItem("fontPrecioProds") > 14) {
			fontPrecioProds = `font-size: ${localStorage.getItem("fontPrecioProds")}px;`;
		...
		if (localStorage.getItem("boldPrecioProds") != null && localStorage.getItem("boldPrecioProds") != 'S') {
			fontPrecioProds += localStorage.getItem("boldPrecioProds") == "N" ? " font-weight: unset;" : "";
		}

		if (disenoProds == 1) {
			estructuraHtml += `<div 
					class="producto-menu pb-2 px-1"
					style="max-width: 140px !important; height: ${baseHeight}px; width: 150px; cursor:${productoValido ? 'pointer' : 'no-drop'}"
				...
				<div class="card mb-2 h-100" style="box-shadow: 2px 4px 3px 1px rgb(69 90 100 / 30%);">
					<div class="card-body p-2 ${op.NombreSubGrupo ? 'd-flex align-items-center justify-content-center' + (op.Imagen ? ' flex-column' : '') : ''}" style="border-radius:3px; background-color: ${colorFondo};">
						${op.Imagen ? `<div class="container-item-prod text-center" style="min-height: 58px; max-height: 58px;">
							<img src="${op.Imagen}" style="max-width: 90px;z-index: 9; height: 100%; border-radius: 3px;">
						</div>` : ''}
						<div class="text-center texto-truncado" style="${!op.Imagen ? '-webkit-line-clamp: 4;' : ''}">
							<p class="m-0 fontProds" style="${fontProds}">${(op.NombreSubGrupo || op.nombre)}</p>
						</div>
						${!op.NombreSubGrupo ? `<div class="mt-1" style="position: absolute; bottom: 3px; right: 8px;">
							<p class="fontPrecioProds" style="${fontPrecioProds}">$${addCommas((+op.Valor).toFixed(0))}</p>
						</div>` : ''}
					</div>
				</div>
			</div>`;
			...
			estructuraHtml += `<div 
					class="producto-menu pb-2 px-1"
					style="max-width: 250px !important; height: ${baseHeight}px; width: 250px; cursor:${productoValido ? 'pointer' : 'no-drop'}"
					id="produ${op.ProductoId}"
					data-valido="${productoValido}"
					data-produ="${op.ProductoId}"
					data-subgrupo=${op.AplicaSubGrupo}
				...
				<div class="card mb-2 h-100" style="box-shadow: 2px 4px 3px 1px rgb(69 90 100 / 30%);">
					<div class="row no-gutters" style="height: 100%; background-color: ${colorFondo}; border-radius:3px;">
						<div class="${op.Imagen ? 'd-flex align-items-center' : 'd-none'} col-4">
							${op.Imagen ? `<div class="container-item-prod text-center" style="min-height: 75px; max-height: 75px;">
								<img src="${op.Imagen}" style="max-width: 90px; z-index: 9; height: 100%; border-radius: 3px;">
							</div>` : ''}
						</div>
						<div class="${op.Imagen ? 'col-8' : 'col-12'}">
							<div class="card-body p-2 ${op.NombreSubGrupo ? 'd-flex align-items-center justify-content-center' + (op.Imagen ? ' flex-column' : '') : ''}">
								<div class="text-center texto-truncado" style="${!op.Imagen ? '-webkit-line-clamp: 4;' : ''}">
									<p class="m-0 fontProds" style="${fontProds}">${(op.NombreSubGrupo || op.nombre)}</p>
								</div>
								${!op.NombreSubGrupo ? `<div class="mt-1" style="position: absolute; bottom: 3px; right: 8px;">
									<strong class="fontPrecioProds" style="${fontPrecioProds}">$${addCommas((+op.Valor).toFixed(0))}</strong>
								</div>` : ''}
							</div>
						</div>
					</div>
	...
	if (!datos.length) {
		$("#productosMenu").append(`<div class="col-12 text-center">No hay productos disponibles</div>`);
	} else {
		if (valorBuscarProducto == '') {
		...
		setTimeout(() => {
			if ($(this).data('subgrupo') > 0) {
				...
				if ($(this).data('valido')) {
					...
					productoActual = { ...datos.find(it => it.ProductoId == $(this).data('produ')) };
					if ((+productoActual['Valor']) <= 0 && $DATOSMONTAJE['ventaceros'] != 'S' && tipoVentaSeleccionado.PreciAbier != 'S') {
...
function validarUnificarMesaAutomatico(MesasCuenta, tercero) {
	let mesasFun = (typeof MesasCuenta == "string" ? MesasCuenta.split(' ').join('').split('-') : [MesasCuenta + ""]);
	let dataUnificar = {
		mesaOriginal: idMesaActual,
		mesasModificar: mesasFun,
		tercero: tercero,
	...
	if ($DATOSMONTAJE.FactConAbier == 'S') {
		...
		setTimeout(() => {
			alertify.confirm('Alerta', `El tercero tiene cuentas en la(s) mesa(s) ${MesasCuenta}, ¿Desea unificar con la mesa ${idMesaActual}?`, function (ok) {
				obtenerInformacion(dataUnificar, 'unificarMesasTercero', 'validarVendedor')
			}, function (err) {
				$("#zonaMesa").change();
				terceroIdPedido = null;
				accionTercero = null;
...
function organizarAcumulado() {
	...
	arrayProductosPedido.forEach((item, pos) => { 
		...
		if (item.valorUnitario) {
			valorProd = +item.valorUnitario * (+item.Cantidad);
		}

/* 		if (item.PromoDescuen) {
			valorProd = ((+item.valorUnitario) * (+item.Cantidad)) - (+item.PromoDescuen);
		} */	
		...
		let color = '#ffffff';
		let letra = 'N';
		if (item.Imprime == 'P' && item.Despachado == 'P' && $INFOALMACEN.NoFisico != 'S') {
			color = '#ffa0a0';
			letra = 'P';
		...
		if ($(".btnOcultarConsumos").hasClass('ocultarConsumos')) {
			if (reservaHotel && reservaHotel != -1 && tipoVentaSeleccionado.Propina == "S" && dataFechasHotel.EntradaReservaHotel) {
				if (item.SedeAlmacen != $INFOALMACEN.SedeId || moment(fechaConsu).isBefore(dataFechasHotel.EntradaReservaHotel) || moment(fechaConsu).isAfter(dataFechasHotel.SalidaReservaHotel)) {
					claseVer = 'd-none';
				}
			} else {
				if (accionPos == 'pedido_mesa') {
					...
					if ((item.MesaId && item.MesaId != '') || (item.AlmacenId + '').trim() != ($INFOALMACEN.almacenid + '').trim()) {
						claseVer = 'd-none';
					}
				}
			}
		...
		if (item.ProductosFamilia && item.ProductosFamilia.length) {
			item.ProductosFamilia.forEach((it, pos2) => {
				estructFami += `<div class="d-flex align-items-center justify-content-between py-1 platos-pedido-item-fami" style="background-color: ${color}">
					<div class="col-1">
						${eliminarProductoPermiso ? `<button data-posi="${pos2}" data-posipadre="${pos}" data-id="${it.Id || 0}" data-pendiente="${item.pendiente ? 'S' : 'N'}" class="btnEliminarProductoFami btn btn-danger btn-xs mb-2" title="Eliminar" style="display: none;">
		...
		$('.platos-pedido').append(`
			<div class="${claseVer} align-items-center justify-content-between py-1 platos-pedido-item" style="background-color: ${color}" data-idcom="${item.Id}" data-consumoid="${item.ConsumoId}" data-valido="${item.nuevo ? 'N' : 'S'}" data-prod="${item.ProductoId}" ${item.ProductosFamilia ? `id="famiHead${item.ProductoId}${pos}" data-target="#famiCollap${item.ProductoId}${pos}" data-toggle="collapse"` : ''}>
				<div class="col-2 px-1">
					<p class="m-0 font-weight-600" style="margin-bottom: -2px; font-weight: 100;">
						${eliminarProductoPermiso ? `<button data-posi="${pos}" class="btnEliminarProducto btn btn-danger btn-xs mb-2" title="Eliminar">
							<i class="fas fa-trash"></i>
						</button>` : ''}
						${restarProductoPermiso && !item.ComandaEditar ? `<button data-posi="${pos}" class="btnEditarProducto btn btn-info btn-xs mb-2 ${item.ProductosFamilia && item.ProductosFamilia.length ? 'd-none' : ''}" title="Editar">
							<i class="fas fa-pencil-alt"></i>
						</button>` : ''}
						${item.estructuraConsumo > 0 && detalleProductoPermiso && !item.ProductosFamilia ? `<button data-posi="${pos}" class="btnDetalleEstructura btn btn-secondary btn-xs mb-2" title="Detalle">
								<i class="fas fa-info-circle"></i>
							</button>` : ``}
						${item.ConsumoId && darBajaProductoPermiso && item.pendiente == 0 ? `<button data-posi="${pos}" class="btnDarDebaja btn btn-warning btn-xs mb-2" title="Dar de baja">
							<i class="fas fa-thumbs-down"></i>
						</button>` : ''}
						${item.ProductosFamilia && item.ProductosFamilia.length ? `<button data-posi="${pos}" data-posfami="${item.posFamilia}" class="btnEditarFamilia btn btn-secondary btn-xs mb-2 d-none" title="Editar Familia">
					...
					<p class="m-0 font-weight-600">
						...
						${$carritoFlotante == 'S' ? `<span class="${letra != 'P' ? 'text-muted' : ''} mb-0">
							...
							<span class="${letra != 'P' ? 'text-muted' : ''} mb-0"> 
								${$DATOSMONTAJE.FactConAbier == 'S' && item.MesaId ? 'Mesa ' + item.MesaId + ' - ' : ''}  
								${(moment(item.FechaConsu).format('DD-MM-YYYY hh:mm:ss A') || moment().format('DD-MM-YYYY hh:mm:ss A'))}
							</span>` : ''}

						${tercerosDesayuno.length ? `<br><span class="${letra != 'P' ? 'text-muted' : ''} mb-0"> 
							${item.NombreTercero}
						</span>` : ''}
					</p>
				</div>
				<div class="text-left col-1 px-0">
					<p class="m-0 font-weight-600 text-center">
						<span class="${letra != 'P' ? 'text-primary' : ''}">${$carritoFlotante == 'N' ? 'Cant: ' : ''}${+item.Cantidad}</span>
					</p>
				</div>
				<div class="text-right col-2 px-0 mr-2">
					<p class="m-0 font-weight-600">
	...
	if (datosTotales.totalSoloPendientes > 0) {
		$(".totalPedidosPendientes").text('$' + addCommas(datosTotales.totalSoloPendientes.toFixed(2)));
...
function cargarAccionTerc(enc) {
	numeroPersonasPedido = $("#NumeroPersonas").val() == '' ? 1 : $("#NumeroPersonas").val();
	let maxPer = $(".mesaId.mesa-seleccionada").data('personas') ? $(".mesaId.mesa-seleccionada").data('personas') : null;
	$("#cantPersonas").html(`<input type="number" name="NumPerEditar" style="color: #ffffff;" class="form-control form-control-floating" 	placeholder="Número de personas" id="NumPerEditar" data-campodb="Personas" value="${numeroPersonasPedido}" max="${maxPer}" min="1" required>
	`);
	accionCamposPedido();
	...
	if ($TIPOCOMERC == 'CLUB' && enc.ZonaCuenta != null) {
		alertify.warning("El tercero tiene cuentas en las siguientes zonas: " + enc.ZonaCuenta);
	}
	if ($TIPOCOMERC == 'CLUB' && enc.CantConsu > 0 && enc.MesasCuenta != null) {
		alertify.warning("El tercero tiene cuentas en las mesas: " + enc.MesasCuenta);
	}
	if (enc.CantConsu > 0 && enc.CantTercIgual == 0 && idMesaActual && consumo != 0) {
		alertify.confirm('Alerta', `La mesa ${idMesaActual} tiene varias cuentas registradas. ¿Desea agregar una cuenta nueva?`, function (ok) {
			...
			if ($TIPOCOMERC == 'CLUB') {
				if (enc.MesasCuentaUnificar == null) {
					...
					if (accionPos == "pedido_mesa") {
						validarUnificarMesaAutomatico(enc.MesasCuentaUnificar, terceroIdPedido);
					}
				}
			} else {
				validarVendedor();
...
function accionesProducto() {
	...
	$('.btnEliminarProducto').click(function () {
		...
		if (info.Despachado == 'P' && info.Imprime == 'P') {
			...
			if (info.pendiente) {
				alertify.confirm(
					'Alerta',
					'¿Desea eliminar el producto ' + info.nombre + ' de la cuenta pendiente?'
					, function (evt, value) {
						let data = {
							Producto: info.ProductoId,
							Id: info.Id
						}
						obtenerInformacion(data, 'borrarDeCuentaPendiente', 'cuentaPendienteBorrado');
					}
					, function () { }
				);
			} else {
				if (info.Imprime == 'P' && info.Despachado == 'P') {
					...
					if ($DATOSMONTAJE.TiempoDevolConsumo > 0) {
						let tiempoDevolucion = moment(info.FechaConsu).add($DATOSMONTAJE.TiempoDevolConsumo, 'minutes');
						if (tiempoDevolucion.isBefore(moment())) {
							abrirCerrarModal("#modal-solicitar-usuario", "show");
						} else {
							imprimirDevolucionConsumo = true;
	...
	$('.btnDetalleEstructura').click(function () {
		...
		if (arrayProductosPedido[posi]['headprodid']) {
			...
			obtenerInformacion(data, 'estructuraventaProducto', 'datosEstructuraProducto')
		}
	});
...
function infoProductoEditar() {
	...
	$("#PuestoMesa").val((productoActual['PuestoMesa'] ? productoActual['PuestoMesa'] : '').split(' ').join(''));
...
function datosEstructuraProducto({ estructura }) {
	...
	estructura.forEach(item => {
		$('#estructuraPlato').append(`<div class="d-flex align-items-center justify-content-between py-2 border-bottom">
	...
	$("#estructuraObservacion").html("<strong>Observacion: </strong>" + (estructura[0] && estructura[0]['Observacio'] ? estructura[0]['Observacio'] : ''))
	abrirCerrarModal("#datosPlatoEstructura", "show");
}

function datosProducto({ disponibles, noDisponibles, observaciones, agrandamiento, cantEstructura }) {
	...
	if (productoActual.inventames != 'S' && productoActual.estructura == 'S' && itemsObservaciones.length > 0) {
	...
	if (productoActual.inventames != 'S') {
		if (productoActual.estructura == 'S') {
			if (disponibles.length) {
				let tempData = dataFijosVariables(disponibles, "#tituloIngredientes", "#listaIngredientes", productoActual);
		...
		if ($DATOSMONTAJE.InventarioRojo != 'S' && +productoActual.invenactua <= 0) {
			$("#cantProductoCont").hide();
		}
	}
	...
	$('.bd-Variables').unbind().on('shown.bs.modal', function (event) {
		...
		if (!productoActual.PromoProdu) {
			let valor = addCommas(+productoActual.Valor);
	...
	if (agrandamiento) {
		...
		$("#btnAgrandamientos").unbind().click(function () {
			let data = {
				headprodid: productoActual.headprodid,
				producto: productoActual.ProductoId,
				almacen: $INFOALMACEN['almacenid'],
			...
			obtenerInformacion(data, 'obtenerItemsProducto', 'itemsProductoAgrandar');
		});
		$(".btnCancelarCargarAgrandamientos").unbind().click(function () {
...
function reactivarConsumoTerc() {
	...
	$(".accionIdPedido").html((accionTercero || ''));
	terceroIdPedido = datosTerceroReactivar.TerceroID ? datosTerceroReactivar.TerceroID.trim() : null;
	...
	if (productoActual.inventames != 'S' && productoActual.estructura == 'S') {
		if (itemsDisponibleProducto.length) {
			...
			let validos = itemsDisponibleProducto.filter(it => {
				...
				if ($("[name=" + it.productoid + "]").length && $("[name=" + it.productoid + "]").is(':checked')) return true;
			});
			if (validos.length > 0 && !validarEstructuraHijos(validos, cantidad, true)) {
				if (accion == 'sumar') {
					$("#cantMasProd").hide();
					$("#cantProducto").prop('disabled', true);
		...
		if (accion != 'restar' && (+productoActual['invenactua']) <= cantidad) {
			if (accion == 'sumar') {
				...
				$("input[name=Cantidad]").val((+productoActual['invenactua']).toFixed());
			}
			$("#cantMasProd").hide();
			$("#cantProducto").prop('disabled', true);
		} else {
...
function changeInputCantidadLetra(inputId, accion = '') {
	...
	$(document).find(`.${dataStorage}`).css({ "font-size": `${cantidad}px` });

	//Se hace la validación de la altura de la card
	disenoCard = localStorage.getItem("disenoProds") == null ? 1 : localStorage.getItem("disenoProds");
	...
	$(document).find(".producto-menu").css({ "height": `${baseHeight}px` });
}
...
function accionBotonesMesa() {
	$("#btnCambioMesas").click(function () {
		...
		$("#zonaMesa, #btnUnificarMesas, #btnMesasTemporales").prop('disabled', cambiarMesas);
		setearVarMesas(cambiarMesas);
		$("#texto-boton-mesa").text(cambiarMesas ? "Seleccione la mesa a Cambiar" : '');
	...
	$("#btnUnificarMesas").click(function () {
		...
		$("#zonaMesa, #btnCambioMesas, #btnMesasTemporales").prop('disabled', unificarMesas);
		setearVarMesas(unificarMesas);
		$("#texto-boton-mesa").text(unificarMesas ? "Seleccione la mesa a unificar" : '');
	...
	$("#btnMesasTemporales").click(function () {
		...
		$("#zonaMesa, #btnCambioMesas, #btnUnificarMesas").prop('disabled', agregar);
		$(this).data('seleccion', agregar);
		if (!$(".modal-mesas-activar.show").length && !$('#modal-solicitar-usuario.show').length) {
			let datos = {
				zona: $("#zonaMesa").val()
			}
...
function datosMesasTemporales({ datos }) {
	...
	$("#btnCargarMesasTemporales").removeClass("disabled").prop("disabled", false);
	datos.forEach(it => {
		$("#lista-mesas-activar").append(`<div class="form-group col-3" style="cursor: pointer;" >
			<div class="custom-control custom-switch input-mesa-temp" data-mesa="${it.MesaId}" data-origi="${it.Activo}" data-mesaup="${it.Id}" data-consu="${it.CantConsu}" data-modificado="0">
				<input  type="checkbox" ${it.CantConsu > 0 ? 'disabled' : ''} ${it.CantConsu > 0 || it.Activo ? 'checked' : ''} class="custom-control-input ${it.CantConsu > 0 ? 'custom-control-input-warning' : ''}" name="Mesa${it.MesaId}" id="Mesa${it.MesaId}">
	...
	if (!datos.length) {
		...
		$("#lista-mesas-activar").html('<div class="text-center w-100">No hay mesas temporales disponibles</div>');
	}
	...
	$(document).on("change", ".input-mesa-temp", function () {
		...
		$(this).attr('data-modificado', validate == $(this).data("origi") ? '0' : '1');
	});
}
...
function borrarCuentaProductoPedido(resp) {
	...
	if (formValidateDataUser) {
		...
		alertify.confirm('Alerta', mensaje, function (ok) {
			if (arrayProductosPedido[editarProductoPedidoNuevo]) {
				if (arrayProductosPedido[editarProductoPedidoNuevo]['Imprime'] == "P" && arrayProductosPedido[editarProductoPedidoNuevo]['Despachado'] == "P") {
					$("#formObservacionBorrar").submit();
					return
				}
			}
...
function dataTiposDevoluciones({ datos, tipo }) {
	...
	if (datos.length) {
		datos.forEach(item => {
			$('#tipoLista').append(`<div class="col-3 p-1">
				<div class="card mb-1 h-100" style="box-shadow: none !important;">
					<div class="card-body p-1" onclick='tipoDevolucionCortesia(${JSON.stringify(item)}, "${tipo}")' style="border:2px solid #d4d4d4; cursor:pointer; border-radius: 5px; height: 80px;">
						<div class="container-item text-center" style="font-size: 13px; text-overflow: ellipsis; white-space: break-spaces;overflow: hidden;">${tipo === 'cortesia' ? item.TipoCortId + ' - ' : ''} ${item.nombre ? item.nombre : item.Nombre}</div>
					</div>
				</div>
			</div>`);
		});
	...
	$("#tituloModalDevolucion").text("Tipos de " + (tipo == 'cortesia' ? 'Cortesia' : 'Devolución'));
	abrirCerrarModal("#tiposDevolucionCortesia", "show", "#modal-solicitar-usuario", "hide");
}

function tipoDevolucionCortesia(devolucion, tipo) {
	tipoDevolucionSeleccionada = devolucion;
	if (arrayProductosPedido[editarProductoPedidoNuevo]) {
		arrayProductosPedido[editarProductoPedidoNuevo]['tipodevol'] = devolucion.tipodevoid;

		if (formValidateDataUser == 'producto' || formValidateDataUser == 'venta') {
			abrirCerrarModal("#observa-factura-borrar", "show", "#tiposDevolucionCortesia", "hide");
...
function observacionesProducto(lista, producto, datosObservaciones, modal) {
	...
	datosObservaciones.forEach(it => {
		...
		$(lista).append(`<div class="col-12 custom-control custom-switch pr-0">
			<input type="checkbox" ${checked ? 'checked' : ''} class="custom-control-input" name="${it.EstructuraId}" id="item${it.EstructuraId}">
			<label class="custom-control-label" for="item${it.EstructuraId}">${it.nombre}</label>
		</div>`);
	});
...
function datosReservas(reservas) {
	...
	let tiempoEspera = (zonaMesaActual['TiempoEsperaReserva'] ? +zonaMesaActual['TiempoEsperaReserva'] : 0);
	reservas.forEach(item => {
		let string = `<a href="#" class="list-group-item list-group-item-action flex-column align-items-start p-2 bor-bottom item-reserva" data-reserva="${item.Id}" onMouseOver="this.style.borderBottomColor = '${item.ColorEstado}'" onMouseOut="this.style.borderBottomColor = '#e1e1e1'">
			<div class="d-flex justify-content-between">
				<div>
					<div class="d-flex w-100 justify-content-between align-items-center">
				...
				${tabReserva == 'proximas' ? `<i class="fas fa-clock m-1 align-self-center text-primary"></i>` :
				tabReserva == 'confirmadas' ? `<i class="fas fa-check m-1 align-self-center text-primary"></i>` :
					tabReserva == 'superadas' ? `<i class="fas fa-times m-1 align-self-center text-danger"></i>` : ''
			}
			</div>
		</a>`;
		let horaMas = moment(item.Inicio).add(tiempoEspera, 'minutes');
		if (tabReserva == 'proximas' && item['Estado'] == 'CN' && moment().isBefore(horaMas)) {
			estructura += string;
		} else {
			if (tabReserva == 'confirmadas' && (moment().isBefore(moment(item.Fin)) && item['Estado'] == 'EM')) {
				estructura += string;
				...
				if (item['Estado'] == 'CN' && item.consumo == null) {
					...
					obtenerInformacion(data, 'modificarEstadoReserva', 'estadoReservaModificado');
				} else if (tabReserva == 'superadas' && item['Estado'] == 'CC' && item['Cancelacion'] == null) {
					estructura += string;
				}
			}
		}
	...
	if (!reservas.length) {
		$(`#lista-reservas-${tabReserva}`).html('<div class="text-center">No hay reservas disponibles</div>');
	} else {
		$(`#lista-reservas-${tabReserva}`).html(estructura != '' ? estructura : '<div class="text-center">No hay reservas disponibles</div>');
		$(".item-reserva").click(function () {
			reservaActual = reservasActuales.find(op => op.Id == $(this).data('reserva'));
			if (reservaActual['Estado'] == 'CN' || reservaActual['Estado'] == 'EM') {
				if (tabReserva == 'proximas') {
					alertify.confirm('Alerta', `¿La persona ${reservaActual['nombre']} ha llegado a la reserva?`, function (ok) {
						let data = {
					...
					$("#cuentasPendienteMesa").html('');
					abrirCerrarModal('.bd-accion-modal-sm', 'show');
					let mesa = mesasPlano.find(it => it['MesaId'] == reservaActual['MesaId']);
					if (mesa) {
						$("#NumeroPersonas").attr('max', mesa.MaximoPersonas);
					}
				}
			} else {
				let next = moment().isAfter(moment(reservaActual.Inicio).add(tiempoEspera, 'minutes'));
				if (next) {
					alertify.warning("La reserva ya sobrepaso el tiempo maximo de espera.")
					return;
				}
			}
		});
...
function facturarPedido(consumoActualesPedido = []) {
	let data = {
		...
		, mesaId: idMesaActual
		, terceroIdPedido
		, terceroId: (terceroPedidoEmpresa['TerceroID'] ? terceroPedidoEmpresa['TerceroID'] : terceroIdPedido)
		, CodigEstru: (tipoVentaSeleccionado.CodigEstru || null)
		, reservaHotel: (accesoCargarCuentaHotel && reservaHotel > 0 ? reservaHotel : null)
		, habitacionHotel: (accesoCargarCuentaHotel && reservaHotel > 0 && habitacionHotel && habitacionHotel != '' ? habitacionHotel : null)
		, AlmacenNoFisico: $INFOALMACEN.NoFisico
		, almacenid: $INFOALMACEN['almacenid']
...
function redirigirFacturar(resp) {
	...
	$.redirectPost(base_url() + 'Administrativos/Servicios/EstadoCuenta/Facturador', {
		...
		, habitacionHotel: !habitacionHotel || habitacionHotel == '' ? null : habitacionHotel
...
function guardarCuenta() {
	...
	if (accesoCargarCuentaHotel && tercerosDesayuno.length) {
		if (arrayProductosPedido.length) {
			...
			return alertify.warning("No se encontraron desayunos agregados al pedido");
		}
	}
...
function guardarProducto() {
	if (accesoCargarCuentaHotel && tercerosDesayuno.length) {
		...
		if (prodsAgregarNew.length) {
			...
			obtenerInformacion(infor, "agregarProductoCuenta", "productoAgregadoCuenta");
		} else {
...
function modalaccionesTercero(acciones, elemento = null) {
	...
	$("#accionesCargarCuenta").html(estructura);
	abrirCerrarModal(".bd-accion-modal-sm", "hide", "#modal-accion-tercero", "show");
		...
		if (elemento) {
			...
			if (tercerosAccion[pos]['TipoDocumento'] == '31') {
				...
				if (cambiarSocioCuenta) {
					...
					$('.empresa-datos').html(`<strong>Empresa:</strong> ${tercerosAccion[pos]['Nombre']} - <strong>Nit:</strong> ${tercerosAccion[pos]['TerceroID']} - <strong>Acción:</strong> ${accionTercero}`);
...
function darBajaProductoPedido(resp) {
	abrirCerrarModal("#modal-solicitar-usuario", "hide");
	alertify.confirm('Alerta', `¿Esta seguro de dar de baja ${formValidateDataUser == 'venta' ? 'a los productos' : 'al producto'}? <br> El comprobante de Dar de Baja se podra visualizar una vez.`, function (ok) {
		let data = {
			tercero: terceroIdPedido,
			mesa: idMesaActual,
			accion: accionTercero,
		...
		if (formValidateDataUser === 'producto') {
			data['Id'] = arrayProductosPedido[editarProductoPedidoNuevo]['Id'];
			data['Producto'] = arrayProductosPedido[editarProductoPedidoNuevo]['ProductoId'];
			data['Pendiente'] = arrayProductosPedido[editarProductoPedidoNuevo]['pendiente'];
		} else {
			let prods = [];
...
function impresionDarDeBaja(data) {
	if (data.idConsumos.length) {
		alertify.success(data.mensaje, 1, () => {
			...
			abrirReporte(`${base_url()}reportes/imprimirDarDeBaja/${insertados}/${tipoVentaSeleccionado.codiventid}/${tipoVentaSeleccionado.impresion}`);
		});
	} else {
...
function socioValidoConfirmarConsumo(resp) {
	alertify.confirmacionConsumo('Consumo de ' + $(".clientePedido:first").text());
}
...
alertify.dialog('confirmacionConsumo', function () {
	return {
		setup: function () {
...
function irAceptarRechazarConsumo(valorConfirmacion) {
	alertify.confirm(
		'Alerta',
		`¿Esta seguro de ${valorConfirmacion ? 'aceptar' : 'rechazar'} el consumo de ${$(".clientePedido:first").text()}?`
		, function (evt, value) {
			let data = {
...
function itemsProductoAgrandar(resp) {
	...
	disponibles.forEach(item => {
		let prod = itemsDisponibleProducto.find(it => it.productoid == item.ProductoAgrandar);
		let check = false;
		if (prod && prod.ActualAgrandamiento || item.elegido) check = true;
		$("#listaAgrandamientos").append(`<div class="col-12 custom-control custom-switch pr-0">
			<input type="checkbox" ${check ? 'checked' : ''} class="custom-control-input" name="${item.ProductoAgrandar}" id="item${item.ProductoAgrandar}">
			<label class="custom-control-label" for="item${item.ProductoAgrandar}">${item.nombre}</label>
		</div>`);
	});
	$("#formAgrandamientos").submit(function (e) {
		...
		itemsDisponibleProducto = itemsDisponibleProducto.filter(it => !it.ActualAgrandamiento).concat(data);
		abrirCerrarModal(".modal-agrandamientos", "hide", ".bd-Variables", "show");
	});
}
...
function datosHabitacionBuscar({ datos }) {
	...
	if (datos.length) {
		...
		$("#HabitacionPedido").unbind().change(function () {
			$("#desayunoHabitacion").html('');
			$('.empresa-datos').html(``);
			terceroPedidoEmpresa = {};
			if ($("#HabitacionPedido").val() != "") {
				let hab = datos.find(op => op.HabitacionId == $("#HabitacionPedido").val());
				if (hab.AplicaDesayuno == "S") {
					$("#desayunoHabitacion").html(`<b>Desayunos: ${+hab.TotalDesayuno}/${+hab.TotalTerceros}</b>`);
				}
				$("#btnBuscarTercero").click();
			} else {
		...
		$("#btnBuscarHabitacion").unbind().click(function () {
			$("#HabitacionPedido").change();
		});
		$("#HabitacionPedido").change();
	} else {
		$("#HabitacionPedido").html('<option value="">No hay habitaciones disponibles</option>');
	}
}
...
function validarPromociones(promo, extra = {}) {
	...
	if(promocion != undefined){
		...
		if(promocion['promoProd'].length != 0){
			for(var promo of promo['promoProd']){
				if (promo['Tipo'] == 'RANC') {
					...
					for(var i = 0; i < promo.Promos.length; i++){
						if (cant >= parseInt(+promo.Promos[i].CantidadInicial) && cant <= parseInt(+promo.Promos[i].CantidadFinal)) {
							...
							if (+promo.Promos[i].Porcentaje || +promo.Promos[i].Porcentaje > 0) {
								let valor = ((+productoActual.ValorOriginal * +promo.Promos[i].Porcentaje) / 100);
				...
				if (promo['Tipo'] == 'OBSE' && validarDesc == 0) {
					...
					let cantProd = parseInt(cant / (+promo.PorCada)) * (+promo.CantidadObsequio);
					if (promo.datosProd) {
						if (promo.datosProd.inventames == "N" && promo.datosProd.estructura == "S") {
							if (!validarEstructuraHijos(promo.datosProd.inventario, cantProd)) {
								alertify.warning("No es posible agregar el producto, no cuenta con inventario suficiente.");
							}
						} else {
							if (promo.datosProd.inventames == "S" && +promo.datosProd.invenactua < cantProd) {
								alertify.warning("No es posible agregar el producto, no cuenta con inventario suficiente.");
							}
						}
					}
			...
			const valoresSinCero = [promoSelect.Ranc.valorProductoRanc1, promoSelect.Ranc.valorProductoRanc2, promoSelect.Valo.valorProductoValo, promoSelect.Desc.valorProductoDesc]
			.filter(valor => valor !== 0 && typeof valor !== 'undefined' && valor !== null);
...
function validarBtnHotelCuentaPendiente() {
	$("#btnCuentaHotel").hide();
	if (cargarCuentaHotelPermiso && dataTerceroPendiente.ReservaHotel && dataTerceroPendiente.ReservaHotel != -1) {
...
function almancesNoFisicos({ almacenes }) {
	if (almacenes.length == 1) {
		...
		almacenes.forEach(op => {
			estructura += `<div onclick='almacenNoFisico(${JSON.stringify(op)})' title="Ir al almacen" class="m-b-10 d-flex border rounded justify-content-between align-items-center pl-2">
				<div style="width: 72%;">
...
function almacenNoFisico(almacen) {
	if (!almacen.TipoMenuId) return alertify.error("Este almacen no tiene tipo de menú");
...
function informacionFamiliaProducto({ datos }) {
	...
	datos.forEach((op, index) => {
		$("#lista-tipo-comidas").append(`<div class="col-4 col-md-3 col-sm-4 col-lg-2 col-xl-2 card-tipo-familia px-1 mb-2" style="cursor:pointer; height: 100px; max-height: 100px;" data-familia="${op.FamiProdId}" data-activo="${(index == 0 ? 'S' : 'N')}" data-veces="${op.VecesFami}" data-pos="${index}" title="${op.Nombre}">
			<div class="card mb-1 h-100" style="box-shadow: none !important;">
				<div class="card-body px-1 py-1" style="border:1px solid #b2b9be;">
					<div class="d-flex flex-column align-items-center justify-content-center h-100">
						${op.Imagen ? `<img src="${op.Imagen}" style="width:auto; height: 75%; border-radius: 3px;max-width: 100%;">` : ''}
						<p class="text-center ${op.Imagen ? 'text-truncate m-0' : 'm-2'}" style="${op.Imagen ? 'width: 95%; height: 25%;' : 'font-size: 16px;'}">${op.Nombre}</p>
					</div>
				</div>
			</div>
		</div>`);
	...
	$("#btnAccionesMesa button, .formBuscarProductos input").prop('disabled', true);
	...
	$(".card-tipo-familia").unbind().click(function () {
		$(".formBuscarProductosFamilias, .platosFamiliaCant").hide();
		$("#valProdFami").val('');
		$('.card-tipo-familia').attr('data-activo', 'N').children().children().removeClass('producto-seleccionado');
		$(this).attr('data-activo', 'S').children().children().addClass('producto-seleccionado');
		...
		obtenerProducsdDelMenu('informacionProdsFamilia', { familiaId: $(this).data('familia') });
	});
	$("#btnAnteriorFam").unbind().click(function () {
	...
	$("#btnSiguienteFam").unbind().click(function () {
		let posi = $('.card-tipo-familia[data-activo=S]').data('pos');
		let elemento = $('.card-tipo-familia[data-pos=' + (+posi + 1) + ']');
	...
	$("#btnCancelarFam").unbind().click(function () {
	...
	$("#btnAceptarFam").unbind().click(function () {
		...
		if (hayProds) {
			...
			if ((llaves.length - 1) < +$("#cantProducto").val()) {
				prodsSelectFamilia[posPlatoActualFami].forEach(it => it.Cantidad = (+$("#cantProducto").val() - (llaves.length - 1)));
			} else {
				prodsSelectFamilia[posPlatoActualFami].forEach(it => (editarProductoPedidoNuevo > -1 ? null : it.Cantidad = 1));
			}
			let prodsBuscar = {};
			Object.keys(prodsSelectFamilia).forEach(key => {
				prodsBuscar[key] = prodsSelectFamilia[key].map(({ headprodid, ProductoId }) => {
	...
	$("#btnConfirmPlatoFam").unbind().click(function () {
		if (!prodsSelectFamilia[posPlatoActualFami].length) {
...
function informacionProdsFamilia({ datos }) {
	...
	$(".no-hay-items").remove();
	if (!datos.length) {
		$("#productosMenu").append(`<div class="col-12 text-center no-hay-items">No hay productos disponibles</div>`);
	} else {
		datos.forEach(op => {
			...
			let selected = '';
			let pos = prodsSelectFamilia[posPlatoActualFami].findIndex(it => it.ProductoId == op.ProductoId);
			if (pos != -1) {
				selected = 'producto-seleccionado';
				prodsSelectFamilia[posPlatoActualFami][pos].FamiProdId = op.FamiProdId;
			}
			estructuraHtml += `<div 
				class="producto-menu-familia-padre pb-2 px-1" style="max-width: 140px !important; height: 135px; width: 150px; cursor:${productoValido ? 'pointer' : 'no-drop'}; position: relative">
				<div class="producto-menu-familia card mb-2 h-100" style="box-shadow: 2px 4px 3px 1px rgb(69 90 100 / 30%);" id="produFam${op.ProductoId}" data-valido="${productoValido}" data-produ="${op.ProductoId}" title="${op.nombre}" data-familia="${op.FamiProdId}">
					<div class="card-body ${selected} px-2 py-1 d-flex justify-content-center align-items-center" style="border-radius:2px; ${productoValido ? '' : 'background-color: #d6d6d6;'}">
						<p class="m-0 text-center" style="font-size: 12px; overflow-wrap: anywhere;" title=${op.nombre}>${op.nombre}</p>
					</div>
				</div>
				${editarProductoPedidoNuevo != -1 ? '' : `<button class="btn btn-sm bg-danger delete" data-produ="${op.ProductoId}" title="Eliminar" style="position: absolute; right: 0px; top: 0px; ${selected == "" ? "display: none;" : ""}">
					<i class="fas fa-times"></i>
				</button>`}
			</div>`;
		});
		...
		$(".producto-menu-familia").unbind().on('click', function () {
			if ($(this).data('valido')) {
				let contx = this;
				let infoFami = $('.card-tipo-familia[data-activo=S]').data();
				let productoFami = datos.find(it => it.ProductoId == $(this).data('produ'));
				...
				let cantidad = prodsSelectFamilia[posPlatoActualFami].filter(op => op.FamiProdId == $(this).data('familia')).length + 1;
				if (cantidad > infoFami.veces) {
					alertify.warning(`Solo puede seleccionar ${infoFami.veces} producto(s)`);
				} else {
					let data = {
					...
					$.ajax({
						...
						success: (resp) => {
							let { disponibles, cantEstructura, observaciones, noDisponibles } = JSON.parse($.Desencriptar(resp));
							let datosActualizar = {
							...
							if (productoFami.inventames != 'S' && productoFami.estructura == 'S') {
								if (cantEstructura == 0) {
									return alertify.error("El producto no tiene una estructura registrada");
								}
								if (!disponibles.length) {
								...
								datosProductoFamilia({ disponibles, observaciones, noDisponibles }, productoFami, datosActualizar);
		...
		$(".producto-menu-familia-padre .delete").unbind().on('click', function () {
			let index = prodsSelectFamilia[posPlatoActualFami].findIndex(op => op.ProductoId == $(this).data('produ'));
			if (index != -1) {
				prodsSelectFamilia[posPlatoActualFami].splice(index, 1);
				$(`#produFam${$(this).data('produ')}`).children().removeClass('producto-seleccionado');
				$(this).hide();
			}
			platosFamilia();
		});
		$("#valProdFami").unbind().on('keyup', function () {
			let valor = $(this).val();
			...
			$.each($(".producto-menu-familia-padre"), function (pos, it) {
				let temp = $(it).find('p');
				$(it).show();
				if (!temp.text().toLowerCase().includes(valor.toLowerCase())) $(it).hide();
			});
		});
		if (editarProductoPedidoNuevo != -1) {
			platosFamilia();
...
function reorganizarAnteFamilia(eliminar = false) {
	...
	$("#btnAccionesMesa button, .formBuscarProductos input").prop('disabled', false);
	$(".card-tipo-comida, .producto-menu").show();
	if (arrayProductosPedido.length) {
		$(".btn-floating").show();
	}
	$("#btnProcesosFamilias, .formBuscarProductosFamilias, .contenido-plato-familia, .platosFamiliaCant").hide();
...
function validarEstructuraHijos(hijos, cantidad, guardar = false) {
	...
	for (let i = 0; i < hijos.length; i++) {
		if (hijos[i].estructura == "S" && hijos[i].inventames != "S") {
			if (!validarEstructuraHijos(hijos[i].estructuraHija, cantidad, guardar)) return false;
			...
			if (!(val <= (+hijos[i].invenactua)) && $DATOSMONTAJE.verInvEstrVent == 'S') return false;
		}
	}
	return true;
}
...
function dataTipoVenta({ datos }) {
	if (datos) {
		...
		if (tipoVentaSeleccionado.PagoPendiente == 'S' && tipoVentaSeleccionado.vendedor != 'S') {
			alertify.alert('¡Alerta!', '<h3 class="mensaje-alerta">No es posible continuar, el tipo de venta tiene pago pendiente y no solicita vendedor.</h3>', function () {
				window.history.back();
			});
			return;
		}

		if (tipoVentaSeleccionado.Personas != 'S') $("#NumeroPersonas").prop('required', false);
		if (accionPos == 'general') {
			if (reactivarConsumo) {
				consumoReactivar();
			} else if (tipoVentaSeleccionado.manejclien == 'N' && tercerosDesayuno.length <= 0) {
				if (tipoVentaSeleccionado.tercero[0]) {
					tercerosAccion = tipoVentaSeleccionado.tercero;
					terceroIdPedido = tipoVentaSeleccionado.tercero[0].TerceroID;
					accionTercero = (tipoVentaSeleccionado.tercero[0].AccionId || '');
...
function validarIngresoTercero(enc) {
	...
	if (enc.Baloteado == 1) {
		alertify.alert(
			'Alerta',
			(enc.EsCliente == 1 ? '<h3 class="mensaje-alerta">La persona' : 'El socio') + " se encuentra bloqueado por la junta directiva, no se permite su ingreso.</h3>"
		)
		return false;
	}
	...
	if (enc.TipoDocumento != '31' && enc.EsEmpleado == 0) {
		if ($INFOALMACEN['VerificarIngreso'] == 'S') {
			if (!enc.Ingreso && ((!enc.ReservaHotel || enc.ReservaHotel == -1) && !enc.AccionId) && enc.TipoDocumento != '31') {
				alertify.warning("No es posible continuar, no tiene ingreso registrado para hoy.");
				return false;
			} else if (enc.Ingreso && enc.CantidadIngresoSedeActual == 0 && enc.TipoDocumento != '31') {
				alertify.error("El tercero tiene ingreso en otra sede.");
				return false;
			} else {
				if (enc.ReservaHotel && enc.ReservaHotel != -1) {
					if (accesoCargarCuentaHotel && enc.SedeReservaHotel != $INFOALMACEN['SedeId']) {
						alertify.error("La reserva se encuentra en otra sede.");
						return false;
					} else if (enc.SedeReservaHotel != $INFOALMACEN['SedeId'] && !enc.Ingreso) {
						alertify.error("El tercero tiene reserva de hotel en otra sede y no tiene ingreso registrado.");
						return false;
					} else if (!enc.AccionId && enc.SalidaReservaHotel && moment(enc.SalidaReservaHotel, 'YYYY-MM-DD').isBefore(moment(moment().format('YYYY-MM-DD'), 'YYYY-MM-DD'))) {
			...
			if (enc.ReservaHotel && enc.ReservaHotel != -1) {
				if (!enc.AccionId && enc.SalidaReservaHotel && moment(enc.SalidaReservaHotel, 'YYYY-MM-DD').isBefore(moment(moment().format('YYYY-MM-DD'), 'YYYY-MM-DD'))) {
					alertify.warning("Tiene una reserva vencida en Hotel.");
					return false;
...
function cuentasMesaTerceros({ cuentas }) {
	...
	$("#cuentasTerceroMesa").html(estructura);
	if (estructura == '') {
		$("#cuentasTerceroMesa").html('<div class="m-b-10 text-center">No tiene cuentas disponibles</div>');
	}
	abrirCerrarModal("#modal-cambiar-cuenta", "show");
	$(".btnTerceroCambiarCuentaPendiente").unbind().click(function () {
		...
		reservaHotel = null;
		if (cargarCuentaHotelPermiso && enco.ReservaHotel && enco.ReservaHotel != -1) {
...
function productoAgregadoCuenta({ mensaje, productos }) {
	...
	if (!lineaTiempoSubGrupos.length) {
		...
		$(`.item-subgrupo[data-grupo=${lineaTiempoSubGrupos[lineaTiempoSubGrupos.length - 1].GrupoId}]`).click();
	}
	obtenerProductosTercerosDesayuno();
}
...
function dataTercerosDesayuno({ terceros, reserva }) {
	...
	numeroPersonasPedido = 1;
	terceroIdPedido = '0';
	[...btnCarrito, '#btnFacturarPedido'].forEach(op => $(op).addClass('d-none'));
	$("#btnCuentaHotel").show();
	validarVendedor();
	...
	if (prodsAgregarNew.length) {
		...
		terceros.forEach(op => {
			if (!op.ProductoDesayuno.ProductoId) {
				let reserva = tercerosDesayunoOriginal.find(x => x.terceroid == op.TerceroID);
				if (reserva) {
					if (!habitacionesStruc[reserva.reservaId + '']) {
						habitacionesStruc[reserva.reservaId + ''] = '';
					}
					habitacionesStruc[reserva.reservaId + ''] += `<div class="col-12 col-sm-4 col-md-4 col-lg-2 mb-2" style="cursor:pointer">
						<div class="card mb-1 h-100 card-tercero-desayuno" style="box-shadow: none !important;" data-tercero="${op.TerceroID}">
							<div class="card-body px-1 py-1 rounded" style="border:1px solid #b2b9be;">
								<div class="text-center">
									<img src="${!op.Foto ? base_url() + 'assets/images/user/nofoto.png' : op.Foto}" style="height: 68px; border-radius: 3px;">
								</div>
								<p style="text-align: center;font-size: 14px;margin-bottom:6px;margin-top: 3px;">${op.NombreTercero}</p>
								<p class="nameProd" style="text-align: center;font-size: 12px;margin-bottom:6px;">${(op.ProductoDesayuno.nombre || '')}</p>
							</div>
							<button class="btn btn-sm bg-danger deleteDesayuno" data-tercero="${op.TerceroID}" title="Eliminar Desayuno" style="position: absolute; right: 0px; top: 0px; ${!op.ProductoDesayuno.ProductoId ? "display: none;" : ""}">
								<i class="fas fa-times"></i>
							</button>
						</div>
					</div>`;
			...
			if (prodsAgregarNew.length) {
				...
				if (pos > -1) {
					if (!terceros[pos]['ProductoDesayuno'].ProductoId) {
						...
						alertify.warning(terceros[pos].NombreTercero + " ya se le asigno desayuno");
					}
				}
			} else {
				alertify.warning("No se encontraron productos agregados");
		...
		$("#btnElegirTercerosDesayuno").unbind().on('click', function () {
			let dataTerc = [];
			terceros.forEach(x => {
				if (x.ProductoDesayuno && x.ProductoDesayuno.ProductoId && !x.ProductoDesayuno.Id) {
			...
			if (dataTerc.length) {
				dataTerc = dataTerc.map(op => {
					...
					let enc = tercerosDesayunoOriginal.find(x => x.terceroid == op.TerceroId);
					if (enc) {
...
function desayunosPendientesAlert(terceros) {
	...
	if (total == 1) {
		$(".desayunospendientes").html(total + " Desayuno Pendiente").show();
	} else {
		$(".desayunospendientes").html(total + " Desayunos Pendientes").show();
	}
}
...
function obtenerProductosTercerosDesayuno() {
	if (accesoCargarCuentaHotel && tercerosDesayuno.length) {
		...
		obtenerInformacion(infor, "obtenerProductoTercDesayuno", "productosTerceroDesayuno");
	}
}
...
function datosProductoFamilia({ disponibles, observaciones, noDisponibles }, datosProdF, dataActualiza) {
	...
	if (datosProdF.inventames != 'S' && datosProdF.estructura == 'S') {
		let tempData = dataFijosVariables(disponibles, "#tituloIngredientesFamilia", "#listaIngredientesProductoFamilia", datosProdF);
		disponibles = [...tempData.datos];
		mostrarAlerta = tempData.titulo;
	} else {
		$("#tituloIngredientesFamilia").hide();
	}
	$('.datos-prod-familia').unbind().on('shown.bs.modal', function (event) {
	...
	$("#formIngredientesFamilia").unbind().submit(function (e) {
		e.preventDefault();
		if ($(this).valid()) {

			let $fills = $("#formIngredientesFamilia input, #formIngredientesFamilia textarea");
			...
			if (observaciones.length) {
				observacionesProducto("#listaObservacionesFamilia", datosProdF, observaciones, ".modal-observaciones-producto-familia");

				$("#formObservacionesProdFamilia").unbind().submit(function (e) {
					e.preventDefault();
					if ($(this).valid()) {
						...
						$.each($fills, (pos, input) => {
							const name = $(input).attr("name");
							let item = observaciones.find(op => op.EstructuraId == name);
							let itemProd = Object.assign({}, item);
							if ($(input).is(":checked")) {
								itemProd.elegido = true;
								itemProd.checked = true;
						...
						abrirCerrarModal(".modal-observaciones-producto-familia", "hide");
						// Agregamos producto a la estructura
						prodsSelectFamilia[dataActualiza.posicionFami].push(datosProdF);
						$(dataActualiza.contexto).children().addClass('producto-seleccionado');
						$(dataActualiza.contexto).next().show();
						if (dataActualiza.cantidad == dataActualiza.veces) {
							$("#btnSiguienteFam").click();
						}
					} else {
						alertify.warning("Los valores de las opciones no son correctos")
					}
				});
			} else {
				// Agregamos producto a la estructura
...
function dataFijosVariables(datos, titulo, lista, producto) {
	...
	datos.forEach(item => {
		if (item.Tipo != '' && item.Tipo != ' ') {
			...
			ingredientes[tipo] += `<div class="col-12 custom-control custom-switch pr-0">
				<input type="checkbox" ${item.elegido == 1 ? 'checked' : (item.elegido == 0 ? '' : item.Tipo == 'F' || item.checked ? 'checked' : '')} class="custom-control-input" name="${item.productoid}" id="item${item.productoid}">
				<label class="custom-control-label" for="item${item.productoid}">${item.nombre}</label>
			</div>`;
			if (item.elegido == 1) {
				item.checked = true;
				item.elegido = true;
...
function validarProductosFamilia() {
	let totalHay = Object.keys(prodsSelectFamilia).filter(it => prodsSelectFamilia[it].length).length;
	$(".platosFamiliaCant").text(`${+$("#cantProducto").val() - totalHay} Platos Pendientes`)
}
...
function abrirModuloCuentasPendientes() {
	let cargaModulo = $.Deferred(function (dfd) {
		$(".modal-cuentas-pendientes-modulo").load(`${base_url()}Administrativos/Servicios/CuentasPendientes/cargarModuloModal`, {
			almacen: $INFOALMACEN.almacenid,
		}, dfd.resolve);
	}).promise();
