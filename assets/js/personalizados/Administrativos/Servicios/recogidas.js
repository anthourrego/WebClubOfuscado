...
$(document).on('click', '#btnAceptar', function (e) {
	...
	$('.operador').html('-');
	let valor = parseFloat($('[data-valor').val().replace(/,/g, "")) ;
	if(UltimaTecla != 'Restar' && parseFloat($('[data-resultado]').attr('data-resultado')) > 0 && validasuma == 0){
		valor = valor ? valor : '';
		resultado  = resulcadena + valor;
		let data = resultadoCadena(resultado);
	...
	if(UltimaTecla != 'Igual'){
		if (UltimaTecla == 'Sumar' || UltimaTecla == 'Restar') {
			$('[data-valor').val(parseFloat($('[data-resultado]').attr('data-resultado')));
			$('#valorOperacion').val('');
			...
			if($('[data-operacion]').attr('data-operacion') == '0'){
				$('[data-valor').val(parseFloat($('[data-resultado]').attr('data-resultado')) + parseFloat(valor));
			}else{
				$('[data-valor').val((operacion > valor ) ? parseFloat(operacion) + parseFloat(valor) : 0);
			}
			$('[data-resultado]').attr('data-resultado',0);
			resulcadena = '';
		} else if (validaresta == 1){
			...
			if($('[data-resultado]').attr('data-resultado') != '0' && operacion != 0){
	...
	if (value != '') {
		...
		if (value != '') {
			...
			value += '';
			value = value.length > 1 ? value.substring(0, value.length - remover) : '0';
		}
	}
	setTimeout(function () {
		click = true;
...
function GuardarRecogida(param) {
	...
	alertify.alert('Recogida: ' + param.Consecutivo, 'Recogida almacenada satisfactoriamente', function () {
		abrirReporte(base_url() + 'Reportes/ImprimirRecogida/' + param.idisertado, contexto, 'redireccionRecogida');
		location.reload();
	});
}
