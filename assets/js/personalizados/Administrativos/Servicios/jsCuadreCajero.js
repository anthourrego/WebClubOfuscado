...
var contexto = this;
let contentDenominacionesCuadre = {};
let CuadreCajeroValores = {};
alertify.myAlert ||
		...
		success: function (registro) {
			switch (registro) {
				...
				case 2:
					alertify.alert(
						"Advertencia",
						"No se encuentra información para cuadre en el día especificado"
					);
					break;
				case 3:
					alertify.alert(
						"Advertencia",
						"Existen facturas pendientes por cancelar"
					);
					break;
				default:
					if (registro[0] == "1") {
						alertify.alert(
							"Cuadre Cajero",
							"Cuadre de Cajero generado con éxito No " + registro[1],
							function () {
								abrirReporte(
									base_url() + "Reportes/ImprimirCuadreCajero/" + registro[2],
									contexto,
									"redireccionImprimir"
								);
							}
						...
						alertify.alert(
							"Error",
							"Ocurrió un problema al momento de registrar el cuadre",
							function () {
								this.destroy();
							}
						);
		...
		success: function (res) {
			...
			if (TotalRegistro[0] == 1) {
				if (TotalRegistro[1] != null && $clavecuadr == "S") {
					...
					alertify.confirm(
						"Advertencia",
						"Este usuario ya generó el cuadre de cajero para este día. ¿Desea generar uno nuevo? ",
						function () {
							if ($clavecuadr == "S") {
					...
					alertify.alert(
						"Advertencia",
						"Este Usuario ya generó el cuadre de cajero para este día",
						function () {
							$("#valor").val(0);
							$("#tarjetas").val(TotalRegistro[2]);
							$("#creditos").val(TotalRegistro[3]);
				...
				alertify.alert(
					"Error",
					"Ocurrió un problema al momento de cargar la información del cajero",
					function () {
						this.destroy();
					}
				);
...
$(function () {
	...
	$("#frmCuadre").on("submit", function (e) {
		...
		if ($(this).valid()) {
			if ($("#valor").val() == "") {
				alertify.confirm(
					"Advertencia",
					"¿Está seguro de generar el cuadre con valor entregado igual a cero?",
					function () {
						cuadreCajero();
					},
					function () {}
	...
	$("#frmCuadre").on(
		...
		function () {
			...
			if (value != "") {
				...
				$.ajax({
					url:
						base_url() + "Administrativos/Servicios/CuadreCajero/CargarForanea",
					type: "POST",
					data: {
						tabla: tabla,
						value: value,
					...
					success: function (respuesta) {
						if (respuesta == 0) {
							...
							$(self)
								.closest(".input-group")
								.find("span")
								.text(respuesta[0][tblNombre].trim())
								.attr("title", respuesta[0][tblNombre].trim());
							cargarTercero();
						}
					},
				});
	...
	$("#formDataAdmin").submit(function (e) {
		...
		if ($(this).valid()) {
			...
			$.ajax({
				url:
					base_url() + "Administrativos/Servicios/CuadreCajero/validarUsuario",
				type: "POST",
				data: {
					encriptado: datas,
				},
	...
	if ($SolitarDenominacionCuadre == "S") {
		$("#valor").attr("disabled", true);
		$(".formMoney").attr("hidden", false);
	}
});

$("#formMoney").click(function (e) {
	e.preventDefault();
	DenominacionEfectivo({
		efectivo: "",
		tarjetas: $("#tarjetas").val(),
		creditos: $("#creditos").val(),
		contentDenominaciones: JSON.parse(
			JSON.stringify(contentDenominacionesCuadre)
		),
	})
		.then((res) => {
			contentDenominacionesCuadre = { ...res.dataFormulario };
			CuadreCajeroValores = { ...res.dataFinal };
			$("#valor").val(contentDenominacionesCuadre.totalefectivo);
		})
		.catch((error) => {
			console.log(error);
		});
