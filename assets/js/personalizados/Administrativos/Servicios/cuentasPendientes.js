let rutaGeneralCuentasPendientes =
	base_url() + "Administrativos/Servicios/CuentasPendientes/";
...
let tipoVentaSeleccionadoCuentaPendiente = JSON.parse(
	sessionStorage.getItem("tipoVenta")
);
...
function dataTable() {
	...
	if ($TIPOCOMERPENDIENTE == 'AYB') {
		let divTextAlert = `El tipo de venta <b>${tipoVentaSeleccionadoCuentaPendiente.nombre}</b> ${(tipoVentaSeleccionadoCuentaPendiente.manejmesa == 'S') ? "" : "no"} maneja mesas.`
		$("#divAlertTipoVentaAYB").html(divTextAlert).removeClass("d-none");
	}

	dataFiltroCuentaPendiente["almacen"] = $ALMACENCUENTAPENDIENTE;
	dataFiltroCuentaPendiente['codiVent'] = tipoVentaSeleccionadoCuentaPendiente;
	tblCuentasPendientes = $("#tablaCuentasPendientes").DataTable({
		...
		ajax: {
			url:
				rutaGeneralCuentasPendientes +
				($DATOSMONTAJECUENTAPENDIENTE.FactConAbier == "S"
					? "cargarDTAgrupada"
					: "cargarDT"),
			type: "POST",
			data: function (d) {
		...
		createdRow: function (row, data, dataIndex) {
			...
			$(row).on("click", function (e) {
				...
				if (
					tipoVentaSeleccionadoCuentaPendiente.PagoPendiente == "S" &&
					tipoVentaSeleccionadoCuentaPendiente.vendedor != "S"
				) {
					alertify.alert(
						"¡Alerta!",
	...
	$.ajax({
		...
		success: (resp) => {
			...
			if (resp.valido && funcion) {
				...
				if (resp.alerta) {
					alertify
						.alert("¡Advertencia!", resp.mensaje)
...
function datosDeConsumoP(datos) {
	...
	let datosSend = Object.assign(
		{},
		{
			...
			tipoVentaSeleccionado: JSON.stringify(
				tipoVentaSeleccionadoCuentaPendiente
			),
		}
	);
	sessionStorage.setItem("accionPos", datosSend.accionPos);
	datosSend.arrayProductosPedido = JSON.stringify(
		datosSend.arrayProductosPedido
	);
	datosSend["VendedorId"] = vendedorElegido;

	datosSend["terceroPedidoEmpresa"] =
		JSON.parse(datos.dataTerceroPendiente).TerceroID.trim() !=
		datos.TerceroId.trim()
			? datos.dataTerceroPendiente.trim()
			: JSON.stringify({});
	...
	location.href = base_url() + `Administrativos/Servicios/VistaGeneral/Mesas/${datosSend.AlmacenId.toString().trim()}/${tipoVentaSeleccionadoCuentaPendiente.codiventid.toString().trim()}`;
}
...
$(function () {
	...
	if (tipoVentaSeleccionadoCuentaPendiente.vendedor == "S") {
		$(document).on("click", ".card-vendedor-cuenta-pendiente", function () {
			let actual = this;
			let vend = $VENDEDORESCUENTAPENDIENTE.find(
				(it) => it.vendedorid == $(this).data("vendedor")
			);
			if (
				vend.Pass &&
				$DATOSMONTAJECUENTAPENDIENTE.SolicitaClaveVendedor == "S"
			) {
				let mensaje = `
