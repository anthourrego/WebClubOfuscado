...
let TotalIngresoSeleccionados = [];
let InvitadoIngresaSinSocio = null;
let socios = [];
let invitadosDia = 0;
let invitadosMes = 0;
let contInvitados = 0;
...
$FACTURA = {
	...
	Propina: 0,
	entregado: 0,
	VendedorId: null //$DATOSVENDEDOR.vendedorid,
};
...
function calcularValorProductos() {
	...
	const conteoPorTipoInvitado=[];
	usuariosRegistrar.forEach(ingresos => {
		socios.forEach(element => {
			if((element.TerceroID == ingresos) && (element.Tipo == "I")){
				conteoPorTipoInvitado.push(element);
			}
		});
	});

	const resultado = conteoPorTipoInvitado.reduce((acumulador, elemento) => { 
		const key = elemento.TipoInvitadoId; 
		const Valor = elemento.Valor; 
		const ivaid = elemento.ivaid;
		const ProductoId = elemento.ProductoId;
		const TerceroId = elemento.TerceroID;
		const NombreProducto = elemento.NombreProducto;
		if (!acumulador[key]) { 
			acumulador[key] = { TipoInvitadoId: key , Cantidad: 0 , Valor , ivaid, NombreProducto,TerceroId,ProductoId}; 
		} acumulador[key].Cantidad++; 
		return acumulador; 
	}, {}); 
	$PRODUCTOSFACTURAR = Object.values(resultado); 

	$PRODUCTOSFACTURAR.forEach(element => {
		if(element.Valor !=null && element.Valor !='' && element.Valor !='.00' && element.Valor !='0.00' && element.NombreProducto !='' && element.NombreProducto !=null){
			cantTerceros = element.Cantidad ;
			let Total = 0;
			let mostrarValor = '';
			if (cantTerceros > 0) {
				Total = cantTerceros * element.Valor;
				Total = calcularIva(element.ivaid, Total);
				totalFacturar += Total.Valor;
				totalIVA += Total.ivan
				mostrarValor = addCommas2(Math.round(myRound(Total.Valor)));
				$("#productosFacturarLista").append(`
					<div class="col-7 font-weight-bold text-truncate pl-1" title="${element.NombreProducto}">${element.NombreProducto}</div>
					<div class="col-2 font-weight-bold text-truncate" title="${cantTerceros}">${cantTerceros}</div>
	...
	if (cantTerceros > 0 && $DATOSMONTAJE['CobroClub'] == 'S' && $diasNoCobroInvitado == 'N') {
		$("#btnFacturarIngreso").prop("disabled", false);
		propForm(false);
		$("#productosFacturarNoResultados,.divRegistrar").addClass("d-none");
		$("#productosFacturar,.divFacturador").removeClass("d-none");
		...
		$("#btnFacturarIngreso").prop("disabled", true);
		$("#productosFacturarNoResultados,.divRegistrar").removeClass("d-none");
		$("#productosFacturar,.divFacturador").addClass("d-none");
	}
}
...
function validarResoluciones({ datos, dataFactura }) {
	...
	if (resolValida) {
		if (!$PRODUCTOSFACTURAR.filter(x => x.TerceroId && x.TerceroId.length).length) {
			return alertify.warning("No se encontraron personas asignadas al ingreso");
		}

		productosValidosFactura = $PRODUCTOSFACTURAR
			.filter(x => {
				if(x.TerceroId && x.TerceroId.length && x.Valor !=null && x.Valor !='' && x.Valor !='.00' && x.Valor !='0.00' && x.NombreProducto !='' && x.NombreProducto !=null){
					return x;
				}
			})
			.map(op => {
				let Valor = (+op.Valor * op.Cantidad);
				let calculos = calcularIva(op.ivaid, Valor);
				op.PreciPubli = op.Valor;
...
const facturarCuenta = () => {
	AccionId = $('#nroAccion').val();

	datosSocioIngreso = realizarIngreso(true);
	...
	$.ajax({
		...
		dataType: "json",
		data: {
			consumosIngresos: productosValidosFactura,
		...
		success: (resp) => {
			if (resp.success){
				...
				$datosPost = {
					...
					AlmacenId: $DATOSALMACEN.almacenid,
					TerceroId: TerceroFactura.TerceroId,
					tipoVentaSeleccionado: JSON.stringify(tipoVentaActual),
					accionPos: 'ingreso',
					VendedorId: null,
					...
					TerceroIdConsumo: TerceroFactura.TerceroId,
					consumosOcultos: 'false',
					EsIngreso: 'true',
					datosIngreso: JSON.stringify({"IngresoId": datos.IngresoId, "datosSocioIngreso": datosSocioIngreso})
				}
...
function validarTipoVenta() {
	if ($tiposVenta && $tiposVenta.length) {
		...
		alertify.alert('¡Alerta!', "<h3 class='mensaje-alerta'>El almacén no tiene tipo de venta asignado.</h3>", function(){
			$("#btnFacturarIngreso").attr('disabled',true)
		});
	}
}

function dataTipoVenta(datosTipo) {
	if (datosTipo) {
		tipoVentaActual = datosTipo;
		TerceroFactura.TerceroId = $("#terceroFactura").val();
		TerceroFactura.AccionId = $("#nroAccion").val();
		TerceroFactura.barra = $("#terceroFactura option:selected").data("barra");
		let datos = {
...
function redirigirIngresos(directo = false, recargar = true) {
	if (directo) {
		if (recargar) {
			location.reload();
		} else {
						...
						$(row).click(function(){
							if (data[2] == 1) {
								if (data[3] == 0 || INGRESOMESINVITADOS >= 1) {
									...
									TotalIngresoSeleccionados.push($ID);
									socios.push({
										"TerceroID"       : $ID
										,"Tipo"           : "I"
...
function validarIngresoInvitado() {
	let terceroFactura = $("#terceroFactura").val();

	auxInvitadoIngresa = [];
	usuariosRegistrar.forEach(nroDocumento => {
		let array ='';
		array = socios.find(socio => {
			if(socio.hasOwnProperty('TerceroID') && socio.TerceroID.trim() == nroDocumento && socio.Tipo == 'I'){
				return socio;
			}
		})
		if(array){
			auxInvitadoIngresa.push(array);
		}
	});

	if(auxInvitadoIngresa.length >= usuariosRegistrar.length && InvitadoIngresaSinSocio != 'S' && IngresosAnteriores.length == 0){
		$("#btnRegistrar,#btnAgregarInvitado").prop('disabled', true);
	} else if(usuariosRegistrar.length > 0){
		$("#btnRegistrar,#btnAgregarInvitado").prop('disabled', false);
	} else {
		$("#btnRegistrar").prop('disabled', true);
	}

	$("#terceroFactura").html('<option value="" disabled selected>Seleccione una opción</option>')
	TotalIngresoSeleccionados.forEach(ingresos => {
		socios.forEach(element => {
			if((element.TerceroID == ingresos) && (element.Tipo != "I")){
				$("#terceroFactura").append(`<option data-barra="${element.barra}" data-terceroasumenombre="${element.TerceroAumeCartera.Nombre}" data-terceroasume="${element.TerceroAumeCartera.TerceroId}" value="${element.TerceroID}">${element.nombre}</option>`);
			}
		});
	});
	
	setTimeout(() => {
		$("#terceroFactura").val(terceroFactura).change();
	}, 10);

	calcularValorProductos();
}

function realizarIngreso(retornar = false) {
...
//-------------------------------------------------------------------------------------------------------------------
	...
	$(document).on("change", "#terceroFactura", function(e){
		e.preventDefault();
		$("#terceroAsume").html('');
		$(".divAsumeCartera").addClass("d-none");
		let TerceroAume = $('#terceroFactura option:selected').data();

		if ($(this).val() != TerceroAume.terceroasume) {
			$("#terceroAsume").html(TerceroAume.terceroasumenombre);
			$(".divAsumeCartera").removeClass("d-none");
		}
	});
		...
		if ($(this).valid()) {
			...
			$.ajax({
				...
				success: (resp) => {
					...
					IngresosAnteriores = resp.Ingresos;

					resp.Ingresos.forEach(element => {
						TotalIngresoSeleccionados.push(element.TerceroID);
					});

					InvitadoIngresaSinSocio = resp.InvitadoIngresaSinSocio;
					usuariosRegistrar = [];
					socios = [];
					invitadosDia = 0;
					invitadosMes = 0;
						...
						if (resp.ingresoEspecial || resp.ingresoInvitado) {
							...
							resp.msj.forEach(socio => {
								if (socio.Titular) {
									...
									if (socio.Ingreso) {
									...
									} else if (socio.PermitirIngreso <= 0 || socio.bloqueaSocioAusente == 1 || socio.bloqueoCartera == true) {
										let limitUp = (socio.IngresosInformativos == "S" || socio.bloqueoCartera == true) ? "limitUP" : "disabled";
										$("#cardSocio").addClass("bg-c-yellow text-white " + limitUp).data("select", '3');
										let msjBloqueo = 'Límite de ingreso superado';
										if (socio.bloqueaSocioAusente == 1) {
											msjBloqueo = 'Fecha final del Perido superada';
										} else if (socio.bloqueoCartera == true) {
											msjBloqueo = socio.bloqueoCarteraMsj;
										}
										$("#placaS").val(msjBloqueo).prop("disabled", true).next("label").text("Estado:");
										$("#cardSocio").find(".btnVacuna").data('otrobloqueo', true);
										...
										TotalIngresoSeleccionados.push(socio.TerceroID.trim());
									...
									console.log(socio);
									if (socio.Ingreso) {
										css = socio.IngresaActivo ? 'bg-success text-white disabled border' : 'bg-c-yellow text-white disabled';
										select = 3;
										label = 'Ingreso';
										...
										select = 3;
										label = 'Estado';
										value = 'Límite de ingreso superado';
										if (socio.bloqueaSocioAusente == 1) {
											value = 'Fecha final del Perido superada';
										} else if (socio.bloqueoCartera == true) {
											value = socio.bloqueoCarteraMsj;
										}
										otroBloqueo = true;
										if (socio.IngresosInformativos == "S" || socio.bloqueoCartera == true) {
											css = css + " limitUP";
										} else {
											css = css + " disabled";
										}
										...
										TotalIngresoSeleccionados.push(socio.TerceroID); 
									}
									
									$("#contenedorBeneficiarios").append(`
										<div class="col-10 col-sm-6 col-lg-4 items p-1">
											<div class="card widget-statstic-card ${css} ${socio.IngresaActivo ? 'border' : ''} mb-0 info-estado-socio" data-tercero='${socio.TerceroID}' id="card${socio.TerceroID}" data-select='${select}'>
												<div class="row no-gutters">
													<div class="col-md-4">
														<div class="rounded bg-light w-100 text-center">
				...
				if (socio) {
					if (socio.PermitirIngreso <= 0) {
						...
						if ($("#cardSocio").data("select") != '3') {
							...
							TotalIngresoSeleccionados.push($("#cardSocio").data("tercero").toString());
						} else {
						...
						if (card.data("select") != '3') {
							if (card.data("tercero") != '' && card.data("select") == 0) {
								...
								TotalIngresoSeleccionados.push(card.data("tercero").toString());
							} else {
					...
					$.ajax({
						...
						success: function(resp){
							if (resp.success) {
								...
								TotalIngresoSeleccionados.push(terceroId);
								socios.push({
									"TerceroID": terceroId
									,"Tipo": "E"
	...
	$(document).on("click", ".img-persona", function(){
		...
		if (card.hasClass("limitUP") && card.data("tercero") != '') {
			...
			let msjText = "Límite de ingreso superado";
			if (datos.bloqueoCartera == true) {
				msjText = datos.bloqueoCarteraMsj;
			}
			...
			if (card.data('select') == 3) {
				...
				let msj = '';
				if (datos.bloqueoCartera == true) {
					msj = datos.bloqueoCarteraMsj;
				} else if (datos.bloqueaSocioAusente == 0) {
					msj = "Ha superado el límite de ingresos";
				} else {
					msj = "La fecha final de perido ha finalizado";
				}
				alertify.confirm('Advertencia', `<div class='row'>
							<div class='col-4'>
								<div class="rounded bg-light w-100 text-center">
							...
							<div class='col-8'>
								...
								<hr>
								<h3 class='mensaje-alerta alert-warning p-3 rounded'>¿Está seguro de realizar el ingreso? <br> ${msj}.</h3>
							</div>
						</div>`, function(){
					...
					card.find(".form-group-placa input").val(msjText);
			...
			if (card.data('select') != 3) {
				if (card.data("tercero") != '' && card.data("select") == 0) {
					...
					usuariosRegistrar = usuariosRegistrar.filter((i) => i != card.data("tercero"));
					TotalIngresoSeleccionados = TotalIngresoSeleccionados.filter((i) => i != card.data("tercero"));
	...
	$(document).on("click", ".info-estado-socio", function(e){
		e.preventDefault();
		if ($(this).hasClass("disabled")) {
			let terceroId = $(this).data("tercero"); 
			socioBloqueo = socios.find(socio => socio.TerceroID == terceroId);
			if (socioBloqueo.bloqueoCartera == true && socioBloqueo.Ingreso == null) {
				alertify.alert("Advertencia",`El socio <b>${socioBloqueo.TerceroID}</b> - <b>${socioBloqueo.nombre}</b> con la acción <b>${socioBloqueo.AccionId}</b><br><br> ` + socioBloqueo.bloqueoCarteraMsj);
			}
		}
	});
	...
	$("#formDataAdmin").submit(function (e) {
		...
		if ($(this).valid()) {
			...
			data['permiso'] = 2654; //Autoriza Ingreso Tercero Bloqueado por cartera;
			datas = $.Encriptar(data);
			let TerceroId = cardTemporal.data("tercero").toString();
			$.ajax({
				...
				success: (resp) => {
					...
					if (resp.valido) {
						...
						cardTemporal.addClass("bg-success").removeClass("bg-c-yellow").data("select", "1");
						cardTemporal.find(".form-group-placa label").html("Placa");
						cardTemporal.find(".form-group-placa input").val("");
						usuariosRegistrar.push(cardTemporal.data("tercero").toString());
						TotalIngresoSeleccionados.push(cardTemporal.data("tercero").toString());
						validarIngresoInvitado();
	...
	$("#formfacturar").submit(function (e) {
		...
		if ($(this).valid()) {
			...
			alertify.error("Validar la información del campos tercero a facturar ");
		}
	});
