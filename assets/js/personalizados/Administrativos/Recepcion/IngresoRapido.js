...
$(function(){
			...
			$.ajax({
				...
				success: (resp) => {
					...
					if (resp.success) {
						...
						} else if ((resp.limiteYear || resp.bloqueoCartera) && resp.ingresoEspecial === false) {
							alertify.confirm('¡Alerta!', "<h3> ¿Está seguro de realizar el ingreso? <br><br>" + resp.msj + "</h3>", function(){
								if (resp.bloqueoCartera) {
	...
	$("#formDataAdmin").submit(function (e) {
		...
		if ($(this).valid()) {
			...
			let TerceroId = $("#documento").val();

			if (TerceroId.length > 0) {
				$.ajax({
					...
					data: {
						encriptado: datas
						, TerceroId
						, RASTREO: RASTREO('', 'Ingreso Rapido Club')
					},
					success: (resp) => {
				...
				alertify.error("No se ha seleccionado una tercero");
			}
		} else {
